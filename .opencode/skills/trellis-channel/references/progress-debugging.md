...
`trellis channel messages <channel>` 渲染紧凑、人类可读的视图：时间戳、身份标识、kind 和简短正文。它面向扫描通道的运维人员，而非诊断用途。
...
症状：`trellis channel list` 显示 worker 正在运行，但 `messages` 中没有新事件出现，且 `wait` 持续超时。
...
2. **确认 supervisor 和 worker 的 PID 仍在运行。**
   ...
   `trellis channel kill <name> --as <worker> --force` 清理。
...
| 列表中显示幽灵 worker | supervisor 已退出但未清理 | `trellis channel kill <name> --as <worker> --force` |
