...
使用此技能从真实代码库创建或刷新 `.trellis/spec/` 指南。一个有能力 Agent 拥有完整循环：分析仓库、选择 spec 边界、编写文档、验证结果。此工作流不依赖特定的宿主、CLI 或 Agent 品牌。
...
| 编写高信号 Trellis spec 文件 | [references/spec-writing.md](references/spec-writing.md) |
...
- 不在 `.trellis/spec/` 中留占位符文本、空标题或复制的样板内容。

## 完成标准

- `.trellis/spec/` 描述了项目当前的实际状态。
