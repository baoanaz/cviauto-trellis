...
使用单个 agent 作为默认执行模型。agent 可以创建 Trellis 任务以便追踪，但此 skill 不应要求特定平台、CLI 或并行 worker 模型。
...
当 Trellis 任务有用时，编写一份简洁的 PRD，包含以下章节：

```markdown
# 填写 <包或层名称> Trellis Specs

## 目标（Goal）
为 <范围> 编写项目特定的 `.trellis/spec/` 指导。
