...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage Trellis-owned workspace + current-task paths and commit when enabled.
    ...
    via ``get_current_task``). We never `git add` the whole `.cviauto/` tree
    ...
    Honors ``session_auto_commit`` in ``.cviauto/config.yaml``: when set to
    ...
    # is ``.cviauto/tasks/<name>`` (or under archive/) — pass the bare name.
