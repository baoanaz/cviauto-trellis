...
class CLIAdapter:
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'cviauto', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/cviauto-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Devin uses workflow directory: .devin/workflows/cviauto-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Pi uses prompt templates: .pi/prompts/cviauto-<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/cviauto/<name>.md
        """
        if self.platform == "pi":
            prompts_dir = self.get_config_dir(project_root) / "prompts"
            if not parts:
                return prompts_dir
            if len(parts) >= 2 and parts[0] in ("trellis", "cviauto"):
                filename = parts[-1]
                if filename.endswith(".md"):
                    filename = filename[:-3]
                return prompts_dir / f"cviauto-{filename}.md"
        ...
        if self.platform == "devin":
            ...
            if len(parts) >= 2 and parts[0] in ("trellis", "cviauto"):
                filename = parts[-1]
                return workflow_dir / f"cviauto-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] in ("trellis", "cviauto"):
            # Convert trellis/<name>.md to trellis-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"cviauto-{filename}"
            )
    ...
    def get_trellis_command_path(self, name: str) -> str:
        ...
        Note:
            Cursor: .cursor/commands/cviauto-<name>.md
            Codex: .agents/skills/cviauto-<name>/SKILL.md
            Kiro: .kiro/skills/cviauto-<name>/SKILL.md
            Gemini: .gemini/commands/cviauto/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Devin: .devin/workflows/cviauto-<name>.md
            Pi: .pi/prompts/cviauto-<name>.md
            Others: .{platform}/commands/cviauto/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/cviauto-{name}.md"
        elif self.platform == "codex":
            # 0.5.0-beta.0 renamed all skill dirs to add the `trellis-` prefix
            # (see that release's manifest for the 60+ rename entries).
            return f".agents/skills/cviauto-{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/cviauto-{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/cviauto/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "devin":
            return f".devin/workflows/cviauto-{name}.md"
        elif self.platform == "kilo":
            ...
            return f"{self.config_dir_name}/commands/cviauto/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    # as devin for back-compat until users migrate via `cviauto update --migrate`).
    if (
    ...
    # (.agents/skills/cviauto-* dirs) and no explicit platform config dir.
