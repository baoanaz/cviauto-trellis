...
_SEED_EXAMPLE = (
    ...
    "Run `python3 .cviauto/scripts/get_context.py --mode packages` to list available specs. "
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # load specs via the cviauto-before-dev skill instead of JSONL.
    seeded_jsonl = False
    ...
    print("  - Use /cviauto:continue or phase context to decide the next step", file=sys.stderr)
    print("", file=sys.stderr)
...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    if "archived_to" in result:
        ...
        # Try a bookkeeping commit only when explicitly enabled.
...
def _auto_commit_archive(
    ...
    """Stage Trellis-owned task paths and commit after archive when enabled.
    ...
    ``git add -f .cviauto/`` (which would fan out to caches/backups)
    and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.cviauto/config.yaml``: when
