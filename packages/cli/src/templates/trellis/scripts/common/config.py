...
Reads settings from .cviauto/config.yaml with sensible defaults.
"""
...
def _strip_inline_comment(value: str) -> str:
    ...
    Mirrors :func:`common.cviauto_config._strip_inline_comment` so both
...
def get_session_auto_commit(repo_root: Path | None = None) -> bool:
    ...
    Default: ``False`` (local records only — no auto-stage / auto-commit).
    Set ``session_auto_commit: true`` in ``.cviauto/config.yaml`` to opt into
    the legacy behavior where journal/archive files are staged and committed.
...
def get_spec_base(package: str | None = None, repo_root: Path | None = None) -> str:
    """Get the spec directory base path relative to .cviauto/.
