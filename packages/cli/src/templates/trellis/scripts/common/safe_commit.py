...
----------------------
A real user incident: a project's `.gitignore` listed `.cviauto/` (company-wide
...
`git add -f .cviauto/` — which fan-out-included every ignored subtree
(`.cviauto/.backup-*/`, `.cviauto/worktrees/`, `.cviauto/.template-hashes.json`,
`.cviauto/.runtime/`), committing 548 files / 83474 lines of caches/backups.

Design
------
- Scripts only stage SPECIFIC product paths (journal files, index.md, the
  current task dir, the archive dir). Never the whole `.cviauto/` tree.
- If plain `git add <specific>` fails with "ignored by", DO NOT retry with
  ``-f``. The presence of `.cviauto/` in `.gitignore` is treated as user
  intent ("keep .cviauto/ local-only"). The script warns and skips the
...
- The warning includes a negative example: ``Do NOT use `git add -f .cviauto/` ...``
...
# Paths under .cviauto/ that must NEVER be auto-staged. Listed here so the
# warning to the user can show concrete subpaths to ignore individually
# instead of ignoring the whole `.cviauto/` tree.
...
def safe_trellis_paths_to_add(
    ...
    Included:
      - .cviauto/workspace/<developer>/journal-*.md
      - .cviauto/workspace/<developer>/index.md
      - .cviauto/tasks/<task_name>/   (ONLY the current task dir when
    ...
    Excluded (intentionally — these must not be staged):
      - .cviauto/.backup-*, .cviauto/worktrees/,
        .cviauto/.template-hashes.json, .cviauto/.runtime/, .cviauto/.cache/
...
def safe_archive_paths_to_add(
    ...
    `.cviauto/tasks/` subtree the old way (active tasks + archive). New
...
def print_gitignore_warning(paths: list[str]) -> None:
    ...
    ``Do NOT use `git add -f .cviauto/``` — agents reading the warning are
    known to invent that command, which fans out to ignored caches/backups.
    """
    print(
        "[WARN] git add failed because .cviauto/ paths are ignored by your .gitignore.",
        file=sys.stderr,
    )
    print(
    ...
    if paths:
        ...
        print(
            "[WARN]   .cviauto/workspace/<developer>/{journal-*.md,index.md}",
            file=sys.stderr,
        )
        print(
            "[WARN]   .cviauto/tasks/<task-dir>/",
            file=sys.stderr,
        )
        print(
            "[WARN]   .cviauto/tasks/archive/",
            file=sys.stderr,
        )
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Recommended: change your .gitignore from `.cviauto/` to specific",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Or, if you intentionally keep .cviauto/ local-only, set in",
        file=sys.stderr,
    )
    print(
        "[WARN] .cviauto/config.yaml:",
        file=sys.stderr,
    )
    print(
    ...
    print("[WARN]", file=sys.stderr)
    print(
        "[WARN] Do NOT use `git add -f .cviauto/` — it pulls in backups, worktrees,",
        file=sys.stderr,
    )
    print(
