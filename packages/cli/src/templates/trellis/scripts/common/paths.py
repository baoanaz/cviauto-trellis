...
def get_repo_root(start_path: Path | None = None) -> Path:
    """Find the nearest directory containing .cviauto/ folder.
    ...
    Returns:
        Path to repository root, or current directory if no .cviauto/ found.
    """
    ...
    # Fallback to current directory if no .cviauto/ found
...
def normalize_task_ref(task_ref: str) -> str:
    ...
    `.cviauto/tasks/03-27-my-task`, even on Windows. Absolute paths are preserved
...
def get_spec_dir(package: str | None = None, repo_root: Path | None = None) -> Path:
    ...
    Single-repo: .cviauto/spec
    Monorepo with package: .cviauto/spec/<package>
