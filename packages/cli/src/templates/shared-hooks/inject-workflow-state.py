...
Silent exit 0 cases (no output):
  - No .cviauto/ directory found (not a Trellis project)
...
If you have not already loaded Trellis context this session, read the `cviauto-start` skill once.
</cviauto-bootstrap>"""
...
def find_trellis_root(start: Path) -> Optional[Path]:
    """Walk up from start to find directory containing .cviauto/.

    Handles CWD drift: subdirectory launches, monorepo packages, etc.
    Returns None if no .cviauto/ found (silent no-op).
    """
    cur = start.resolve()
    while cur != cur.parent:
        if (cur / ".cviauto").is_dir():
            return cur
        cur = cur.parent
    return None
...
def _resolve_active_task(root: Path, input_data: dict):
    scripts_dir = root / ".cviauto" / "scripts"
...
def load_breadcrumbs(root: Path) -> dict[str, str]:
    ...
    """
    workflow = root / ".cviauto" / "workflow.md"
...
def _read_trellis_config(root: Path) -> dict:
    """Load .cviauto/config.yaml via the bundled cviauto_config helper.

    The helper lives in .cviauto/scripts/common; the hook lives outside the
    ...
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    try:
        from common.cviauto_config import read_trellis_config  # type: ignore[import-not-found]
    except Exception:
        return {}
    try:
...
def _codex_mode_banner(config: dict) -> str:
    ...
    Reads `codex.dispatch_mode` from .cviauto/config.yaml; defaults to
...
def resolve_breadcrumb_key(
    ...
    opt into ``codex.dispatch_mode: sub-agent`` in ``.cviauto/config.yaml``
...
def main() -> int:
    ...
    if task is None:
        ...
        # cviauto-brainstorm + task.py create when user describes real work.
