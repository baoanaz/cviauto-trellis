...
def should_skip_injection() -> bool:
    """Check if any platform's non-interactive flag is set, or if Cviauto
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    """Expose Cviauto session identity to later Claude Code Bash commands.
...
def _resolve_task_dir(trellis_dir: Path, task_ref: str) -> Path:
    ...
    if path_obj.is_absolute():
        return path_obj
    if normalized.startswith(".cviauto/"):
...
def _get_task_status(trellis_dir: Path, input_data: dict) -> str:
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            f"Next-Action: Run `python3 ./.cviauto/scripts/task.py finish` to clear the stale pointer, "
...
def _load_trellis_config(trellis_dir: Path, input_data: dict) -> tuple:
    """Load Cviauto config for session-start decisions.
...
def _collect_spec_index_paths(trellis_dir: Path, allowed_pkgs: set | None) -> list[str]:
    ...
    if guides_index.is_file():
        paths.append(".cviauto/spec/guides/index.md")
    ...
    for sub in sorted(spec_dir.iterdir()):
        ...
        if index_file.is_file():
            paths.append(f".cviauto/spec/{sub.name}/index.md")
            continue
        ...
        for nested in sorted(sub.iterdir()):
            ...
            if nested_index.is_file():
                paths.append(f".cviauto/spec/{sub.name}/{nested.name}/index.md")
...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `python3 ./.cviauto/scripts/task.py list --mine` only if needed."
            )
        except Exception:
            pass
...
def main():
    ...
    trellis_dir = project_dir / ".cviauto"
