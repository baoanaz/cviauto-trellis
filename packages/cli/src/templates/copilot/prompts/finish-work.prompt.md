---
description: "Cviauto Copilot prompt: Finish Work — survey + local archive task + record session journal"
---

# Finish Work

Wrap up the current session: archive the active task (and any other completed-but-unarchived tasks the user wants to clean up) and record the session journal as local Cviauto records. Code commits are NOT done here — those happen in workflow Phase 3.4 before you invoke this prompt.
...
Development Flow (workflow.md Phase 3):
  ...
  3.5 Wrap-up         -> /finish-work (this prompt: survey + local archive + journal)
...
- `/finish-work` — survey + local archive + record session (this prompt)
