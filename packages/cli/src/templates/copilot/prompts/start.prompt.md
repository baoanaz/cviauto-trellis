---
description: "Cviauto Copilot prompt: Start Session"
---

# Start Session

Initialize a Cviauto-managed development session. This platform has no session-start hook, so manually load the equivalent context by following these steps (each one mirrors a section the hook would otherwise inject).
