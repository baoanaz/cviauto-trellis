---
description: "Cviauto Copilot prompt: Multi-Agent Pipeline Orchestrator"
---
...
### Step 1: Understand Cviauto Workflow `[AI]`

First, read the workflow guide to understand the development process:

```bash
cat .cviauto/workflow.md  # Development process, conventions, and quick start guide
```
...
```bash
python3 ./.cviauto/scripts/get_context.py --mode packages  # Discover available spec layers
cat .cviauto/spec/guides/index.md    # Thinking guides
```
...
```bash
python3 ./.cviauto/scripts/multi_agent/plan.py \
  --name "<feature-name>" \
...
```bash
python3 ./.cviauto/scripts/multi_agent/start.py "$TASK_DIR"
```
...
TASK_DIR=$(python3 ./.cviauto/scripts/task.py create "<title>" --slug <task-name>)
```
...
python3 ./.cviauto/scripts/task.py init-context "$TASK_DIR" <dev_type>

# Set branch and scope
python3 ./.cviauto/scripts/task.py set-branch "$TASK_DIR" feature/<name>
python3 ./.cviauto/scripts/task.py set-scope "$TASK_DIR" <scope>
```
...
```bash
python3 ./.cviauto/scripts/task.py validate "$TASK_DIR"
python3 ./.cviauto/scripts/multi_agent/start.py "$TASK_DIR"
```
...
```bash
python3 ./.cviauto/scripts/multi_agent/status.py                    # Overview
python3 ./.cviauto/scripts/multi_agent/status.py --log <name>       # View log
python3 ./.cviauto/scripts/multi_agent/status.py --watch <name>     # Real-time monitoring
python3 ./.cviauto/scripts/multi_agent/cleanup.py <branch>          # Cleanup worktree
```
