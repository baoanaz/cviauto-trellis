...
import { TrellisContext, debugLog } from "../lib/cviauto-context.js";
 ...
 * (`.cviauto/tasks/foo`) or absolute; both are resolved via
 * `ctx.resolveTaskDir`.
 */
...
- Spec files: \`.cviauto/spec/**/*.md\`
- Known issues: \`.cviauto/big-question/\`
  ...
  return {
    "tool.execute.before": async (input, output) => {
      try {
        ...
        // Strip generated/legacy workflow prefixes before matching the agent kind.
        const subagentType = (rawSubagentType || "").replace(
          /^(?:cviauto|trellis)-/,
          "",
        );
