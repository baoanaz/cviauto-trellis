...
import { debugLog } from "./cviauto-context.js";
...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (!taskRef) {
    return (
      ...
      "Next-Action: Classify the current turn before creating any Cviauto task. " +
      "Simple conversation / small task asks only whether this turn should create a Cviauto task. " +
  ...
  if (active.stale || !taskDir || !existsSync(taskDir)) {
    return `Status: STALE POINTER\nTask: ${taskRef}\nNext-Action: Task directory not found. Run: python3 ./.cviauto/scripts/task.py finish`;
  }
  ...
  if (taskStatus === "completed") {
    return `Status: COMPLETED\nTask: ${taskTitle}\nNext-Action: Run /cviauto:finish-work. If the working tree is dirty, return to Phase 3.4 first.`;
  }
  ...
  if (taskStatus === "planning" && !hasPrd) {
    return `Status: PLANNING\nTask: ${taskTitle}\nPresent: ${presentLine}\nNext-Action: Load cviauto-brainstorm and write prd.md. Stay in planning.`;
  }
...
function loadTrellisConfig(directory, contextKey = null) {
  const scriptPath = join(directory, ".cviauto", "scripts", "get_context.py");
...
function checkLegacySpec(directory, config) {
  ...
  const specDir = join(directory, ".cviauto", "spec");
...
function collectSpecIndexPaths(directory, allowedPkgs) {
  ...
  if (existsSync(guidesIndex)) {
    paths.push(".cviauto/spec/guides/index.md");
  }
  ...
  try {
    ...
    for (const sub of subs) {
      ...
      if (existsSync(indexFile)) {
        paths.push(`.cviauto/spec/${sub}/index.md`);
      } else {
        ...
        try {
          ...
          for (const layer of nested) {
            ...
            if (existsSync(nestedIndex)) {
              paths.push(`.cviauto/spec/${sub}/${layer}/index.md`);
            }
          }
        } catch {
...
function readDeveloper(directory) {
  try {
    const content = readFileSync(
      join(directory, ".cviauto", ".developer"),
      "utf-8",
    );
...
function buildCompactCurrentState(ctx, platformInput, specIndexPaths) {
  ...
  const tasksDir = join(directory, ".cviauto", "tasks");
  if (existsSync(tasksDir)) {
    try {
      ...
      lines.push(
        `Active tasks: ${activeTasks.length} total. Use \`python3 ./.cviauto/scripts/task.py list --mine\` only if needed.`,
      );
    } catch {
  ...
  const workspaceDir = join(directory, ".cviauto", "workspace", developer);
  if (developer !== "(not initialized)" && existsSync(workspaceDir)) {
    try {
      ...
      if (journal) {
        ...
        lines.push(
          `Journal: .cviauto/workspace/${developer}/${journal}, ${lineCount} / 2000 lines.`,
        );
      }
    } catch {
...
Cviauto compact SessionStart context. Use it to orient the session; load details on demand.
  ...
  const workflowContent = ctx.readProjectFile(".cviauto/workflow.md");
  if (workflowContent) {
    const allLines = workflowContent.split("\n");
    const overviewLines = [
      "# Development Workflow - Session Summary",
      "Full guide: .cviauto/workflow.md. Step detail: `python3 ./.cviauto/scripts/get_context.py --mode phase --step <X.Y>`.",
      "",
    ];
...
function getTrellisMetadata(metadata) {
  ...
  const cviauto = metadata.cviauto;
  if (cviauto && typeof cviauto === "object") {
    return cviauto;
  }
