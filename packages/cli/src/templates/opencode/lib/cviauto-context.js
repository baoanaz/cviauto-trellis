...
// Debug logging
const DEBUG_LOG = "/tmp/cviauto-plugin-debug.log";
...
// Matches generated Cviauto sub-agents and legacy Trellis sub-agents exactly.
// Used by chat.message plugins to skip injection inside Cviauto sub-agent turns.
const TRELLIS_SUBAGENT_RE = /^(cviauto|trellis)-(implement|check|research)$/;
...
export class TrellisContext {
  ...
  isTrellisProject() {
    return existsSync(join(this.directory, ".cviauto"));
  }
  ...
  resolveTaskDir(taskRef) {
    ...
    return join(this.directory, ".cviauto", "tasks", normalized);
  }
