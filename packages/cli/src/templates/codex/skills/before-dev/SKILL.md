---
name: before-dev
description: "Discovers and injects project-specific coding guidelines from .cviauto/spec/ before implementation begins. Reads spec indexes, pre-development checklists, and shared thinking guides for the target package. Use when starting a new coding task, before writing any code, switching to a different package, or needing to refresh project conventions and standards."
---
...
4. **Read the spec index** for each relevant module:
   ```bash
   cat .cviauto/spec/<package>/<layer>/index.md
   ```
