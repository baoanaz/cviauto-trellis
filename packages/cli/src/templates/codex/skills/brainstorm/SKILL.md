---
name: brainstorm
description: "Guide requirements discovery for a Cviauto task after task-creation consent. Use when the user is ready to clarify requirements before implementation."
---
...
Use this skill only after task-creation consent has been given and the user is ready to enter Cviauto planning.

If no task exists yet, create one:

```bash
TASK_DIR=$({{PYTHON_CMD}} ./.cviauto/scripts/task.py create "<short task title>" --slug <slug>)
```
...
2. Inspect available evidence before asking questions:
   ...
   - related Cviauto tasks, research files, and session history when present
