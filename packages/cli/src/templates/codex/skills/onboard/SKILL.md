---
name: onboard
description: "Interactive three-part onboarding for new team members to the Cviauto AI-assisted workflow system. Covers core philosophy (AI memory, project-specific knowledge, context drift), system structure and command deep-dives, real-world workflow examples, and guideline customization. Use when a new developer joins the project, someone needs to understand the Cviauto workflow, or project guidelines need initial setup."
---
...
**The Solution**: The `.cviauto/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `$start` skill reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.cviauto/spec/` directory contains project-specific guidelines. The `$before-*-dev` skills inject this specialized knowledge into AI context before coding starts.
...
**[2/8] python3 ./.cviauto/scripts/task.py create "Fix bug" --slug fix-bug** - Track work for future reference
...
**[2/4] python3 ./.cviauto/scripts/task.py create "Planning task" --slug planning-task** - Planning is valuable work
...
Check if `.cviauto/spec/` contains empty templates or customized guidelines:

```bash
# Check if files are still empty templates (look for placeholder text)
grep -l "To be filled by the team" .cviauto/spec/backend/*.md 2>/dev/null | wc -l
grep -l "To be filled by the team" .cviauto/spec/frontend/*.md 2>/dev/null | wc -l
```
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Cviauto in this project.

Explain to the developer:

"I see that the development guidelines in `.cviauto/spec/` are still empty templates. This is normal for a new Cviauto setup!
...
For example, for `.cviauto/spec/backend/database-guidelines.md`:
...
I recommend reading through `.cviauto/spec/` to familiarize yourself with the team's coding standards."
...
```bash
python3 ./.cviauto/scripts/task.py create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
"You're now onboarded to the Cviauto workflow system! Here's what we covered:
...
2. [If guidelines empty] Start filling in `.cviauto/spec/` guidelines
