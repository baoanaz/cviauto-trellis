...
```bash
cat .cviauto/spec/<category>/<file>.md
```
...
- `$update-spec` - Proposes and makes spec updates after user confirmation (this skill)
- `$finish-work` - Can route here when the user explicitly asks to promote a lesson into spec

---

## Core Philosophy

> **Code-specs are living documents. Every debugging session, every "aha moment" can become a clearer implementation contract when a human chooses to promote it.**
