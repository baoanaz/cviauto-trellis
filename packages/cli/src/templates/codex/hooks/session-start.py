...
# -*- coding: utf-8 -*-
"""
Codex Session Start Hook - Inject Cviauto context into Codex sessions.
...
Cviauto SessionStart 已注入：workflow、当前任务状态、开发者身份、git 状态、active tasks、spec 索引已加载。
...
def configure_project_encoding(project_dir: Path) -> None:
    """Reuse Cviauto' shared Windows stdio encoding helper before JSON output."""
    scripts_dir = project_dir / ".cviauto" / "scripts"
...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    if not active.task_path:
        return (
            ...
            "before creating any Cviauto task."
        )
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            "Next: Task directory not found. Run: python3 ./.cviauto/scripts/task.py finish"
        )
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\n"
            f"Next: Archive with `python3 ./.cviauto/scripts/task.py archive {task_dir.name}` "
            "or start a new task."
        )
    ...
    if not has_prd:
        return (
            ...
            "Next: Load cviauto-brainstorm and write prd.md. Stay in planning."
        )
...
Context loaded. Follow <task-status>. Load workflow/spec/task details only when needed.
    ...
    result = {
        "suppressOutput": True,
        "systemMessage": f"Cviauto context injected ({len(context)} chars)",
        "hookSpecificOutput": {
