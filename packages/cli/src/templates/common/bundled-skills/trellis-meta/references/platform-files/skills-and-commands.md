...
Skills 和 commands 是用户与 Cviauto 交互的文本入口。不同平台使用不同的名称，但它们的核心目的相同：告诉 AI 在用户表达特定意图时如何进入 Cviauto 流程。
...
Cviauto 工作流技能通常共享一套语义集合：brainstorm、before-dev、check、update-spec、break-loop。多文件内置技能如 `cviauto-meta` 使用分层引用。
...
它们不应存储任务状态；任务状态属于 `.cviauto/tasks/` 和 `.cviauto/.runtime/`。
