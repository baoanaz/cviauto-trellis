...
当用户想更改 AI 遵循的工程约定、添加新的 spec 层或调整 monorepo 包映射时，编辑 `.cviauto/spec/` 和 `.cviauto/config.yaml`。
...
| 需求 | 编辑位置 |
| --- | --- |
| 添加 backend/frontend/docs/test 的 spec 层 | `.cviauto/spec/<layer>/` 或 `.cviauto/spec/<package>/<layer>/` |
| 添加共享思维指南 | `.cviauto/spec/guides/` |
| 调整 monorepo 包 | `.cviauto/config.yaml` 中的 `packages` |
