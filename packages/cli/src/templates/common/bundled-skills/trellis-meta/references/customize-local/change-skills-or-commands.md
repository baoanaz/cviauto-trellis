...
| 将更改回馈给上游 | 在 Cviauto CLI 仓库中编辑 `packages/cli/src/templates/common/bundled-skills/<name>/`，而非部署后的副本。 |
...
| 分发方式 | 在 `cviauto init` / `cviauto update` 时由 `getBundledSkillTemplates()`（`packages/cli/src/templates/common/index.ts`）自动分发到每个平台 skill 根目录 | 由用户（或其他 skill）创建，永不移动 |
...
- 不要手动编辑任何 `.{platform}/skills/` 目录下的 `cviauto-meta/`、`cviauto-spec-bootstrap/`、`cviauto-session-insight/` 或 `cviauto-channel/` 中的文件并期望修改能持久——它们是捆绑的，会被 `cviauto update` 刷新。要么贡献给上游，要么添加一个补充它们的项目本地 skill。
- 当 `cviauto update` 报告捆绑 skill 文件存在"已被您修改"的冲突时，仅当你接受手动维护差异时才选择**保留**；否则接受覆盖并将意图作为项目本地 skill 重新实现。