...
cviauto channel create brainstorm-storage-layer --by main \
  --task .cviauto/tasks/05-XX-storage-adapter

cviauto channel spawn brainstorm-storage-layer \
  --agent architect --provider codex \
  --file .cviauto/tasks/05-XX-storage-adapter/prd.md \
  --file .cviauto/tasks/05-XX-storage-adapter/design.md \
  --as cx-arch --timeout 30m

cviauto channel send brainstorm-storage-layer \
  --as main --to cx-arch --text-file /tmp/brainstorm-r1.md

cviauto channel wait brainstorm-storage-layer \
...
cviauto channel create cr-foo --task "$TASK" --by main

cviauto channel spawn cr-foo \
  --agent check \
...
cviauto channel send cr-foo --as main --to check --text-file /tmp/cr-brief.md
cviauto channel wait cr-foo --as main --kind done --from check --timeout 15m
...
cviauto channel spawn cr-feature --agent check \
...
cviauto channel send cr-feature --as main --to check --text-file /tmp/cr-brief.md
cviauto channel send cr-feature --as main --to check-cx --text-file /tmp/cr-brief.md
...
cviauto channel thread <board> <thread> --scope global --raw
