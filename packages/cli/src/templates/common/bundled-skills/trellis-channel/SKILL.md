---
name: cviauto-channel
description: Use Cviauto channel for live multi-agent collaboration, spawned workers, cross-agent review, progress inspection, forum channels, and channel log debugging.
---

# cviauto-channel

`cviauto channel` is the local multi-agent collaboration runtime. Reach for it when agents need to talk through a durable event log, when a worker should be spawned as a peer process, when an in-flight worker needs interrupt / debugging, or when feedback should be recorded on a durable `--type forum` channel.
...
cviauto channel thread <board> <thread> --scope global
...
- Use `--context-file` / `--context-raw` and `cviauto channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- `@mindfoldhq/cviauto-core` owns reusable channel/thread state, event append, seq allocation, context/title projection, reducers, and task helpers. The CLI owns flags, terminal rendering, prompts, worker lifecycle, and process exits.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `cviauto mem` (the `cviauto-session-insight` skill) for session/history search.
