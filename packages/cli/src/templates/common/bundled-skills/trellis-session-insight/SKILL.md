...
一个本地 CLI，索引用户过去的 Claude Code、Codex 和 Pi Agent 对话日志（每个平台存储在 `~/.claude/projects/`、`~/.codex/sessions/` 和 `~/.pi/agent/sessions/` 下的 JSONL 文件），并允许你列出、搜索、按 Cviauto 任务边界切片，以及从中导出清洗后的对话内容。OpenCode 日志尚不可索引（provider adapter 待完成）——当 OpenCode 会话是明显目标时，应说明此限制而非猜测。
