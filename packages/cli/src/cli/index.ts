...
/**
 * Check if a Cviauto workflow update is available (compare project version with CLI version)
 */
function checkForUpdates(cwd: string): void {
  ...
  if (comparison > 0) {
    ...
    console.log(
      chalk.yellow(
        `\n⚠️  Cviauto workflow update available: ${projectVersion} → ${cliVersion}`,
      ),
    );
    console.log(chalk.gray(`   Run: cviauto update\n`));
    ...
    console.log(chalk.gray(`   Run: cviauto upgrade\n`));
  }
}

// Check for updates at CLI startup (only if .cviauto exists)
...
program
  .name("cviauto")
  .description(
    "Cviauto AI-assisted development workflow for Claude Code, Codex, and OpenCode",
  )
  .version(VERSION, "-v, --version", "output the version number");

program
  .command("init")
  .description("Initialize Cviauto workflow in the current project")
  .option("--claude", "Include Claude Code commands")
  .option("--opencode", "Include OpenCode commands")
  .option("--codex", "Include Codex skills")
  .option(
    "--with-statusline",
    "Install the Cviauto status line for Claude Code (off by default)",
  )
  ...
  .option(
    "--workflow <id>",
    "Workflow template id for .cviauto/workflow.md (default: native; e.g., tdd, channel-driven-subagent-dispatch)",
  )
  .option(
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      ...
      if (
        process.env.DEBUG ||
        process.env.CVIAUTO_DEBUG ||
        process.env.TRELLIS_DEBUG
      ) {
...
program
  .command("update")
  .description("Update Cviauto workflow configuration and commands")
...
program
  .command("upgrade")
  .description("Upgrade the global Cviauto CLI package")
  .option(
    "--tag <tag>",
...
program
  .command("uninstall")
  .description(
    "Remove all Cviauto workflow files (managed platform files + .cviauto/) from this project",
  )
...
program
  .command("mem")
  .description(
    "Search/recall AI conversation history across Claude Code, Codex, and OpenCode (run 'cviauto mem help' for subcommands and flags)",
  )
...
program
  .command("workflow")
  .description(
    "List or switch the project's .cviauto/workflow.md template (native, tdd, channel-driven-subagent-dispatch, or marketplace)",
  )
  .option(
    "-t, --template <id>",
  ...
  .option(
    "-n, --create-new",
    "Write .cviauto/workflow.md.new instead of replacing the active workflow",
  )
  .action(async (options: Record<string, unknown>) => {
    try {
