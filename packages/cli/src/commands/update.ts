...
function findManagedBlock(content: string): ManagedBlockMatch | null {
...
async function collectRegistrySpecTemplates(
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        `Warning: invalid registry.spec.source in .cviauto/config.yaml: ${
...
async function promptMigrationAction(
  ...
  const whyFlagged = item.reason
    ...
    : chalk.gray(
        `  Why prompted: file content doesn't match the Cviauto template hash\n` +
...
export async function update(options: UpdateOptions): Promise<void> {
  ...
  if (cliVsProject < 0) {
    ...
    if (!options.allowDowngrade) {
      ...
      console.log(
        chalk.gray(`  2. Force downgrade: cviauto update --allow-downgrade\n`),
      );
      return;
    }
  ...
  if (isUnknownVersion) {
    console.log(
      chalk.yellow(
        "⚠️  No version file found. Skipping migrations — run cviauto init to fix.",
      ),
    );
    ...
    if (preConfirmMetadata.breaking) {
      ...
      if (breakingBypass && skipPaths.length > 0) {
        ...
        if (willBypass.length > 0) {
          ...
          console.log(
            chalk.gray(
              "  Hash-verified: only files matching known Cviauto templates are deleted. Your local customizations (hash mismatch) are still preserved.",
            ),
          );
        }
      }
