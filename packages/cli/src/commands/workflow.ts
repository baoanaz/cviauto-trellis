/**
 * `cviauto workflow` command — list and switch the active `.cviauto/workflow.md`.
 *
 * Behavior contracts:
 *
 * - Hash boundary: after writing native content, refresh the
 *   `.cviauto/workflow.md` entry in `.template-hashes.json`. After writing
 *   any non-native content, remove that entry. This prevents `cviauto update`
 ...
 *
 * - `--create-new`: never touches `.cviauto/workflow.md`; writes
 *   `.cviauto/workflow.md.new` and leaves the hash file alone.
 */
  ...
  const { action } = await inquirer.prompt<{ action: string }>([
    {
      ...
      name: "action",
      message:
        "Your .cviauto/workflow.md has local edits. What do you want to do?",
      choices: [
        { name: "Overwrite (replace local edits)", value: "overwrite" },
        {
          name: "Write to .cviauto/workflow.md.new and keep current",
          value: "create-new",
        },
...
function applyHashContract(cwd: string, templateId: string): void {
  ...
  if (templateId === NATIVE_WORKFLOW_ID) {
    ...
    // so `cviauto update` treats it as modified and does not silently restore
    // native bytes.
...
export async function runWorkflowCommand(
  ...
  if (!fs.existsSync(path.join(cwd, DIR_NAMES.WORKFLOW))) {
    throw new WorkflowCommandError(
      "No .cviauto/ directory found. Run `cviauto init` first.",
    );
  }
  ...
  // `.cviauto/agents/<name>.md` files that don't exist on disk, point the user
  // at `cviauto update` so `cviauto channel spawn --agent <name>` doesn't fail
...
function warnAboutMissingAgents(cwd: string, workflowContent: string): void {
  ...
  process.stderr.write(
    chalk.yellow(
      `\n⚠ The selected workflow references .cviauto/agents/{${missing.join(",")}}.md, but those files are not on disk.\n`,
    ) +
      chalk.yellow(
        `  Run \`cviauto update\` to backfill the bundled agent definitions, or create them under ${PATHS.AGENTS}/.\n`,
      ),
  );
}
