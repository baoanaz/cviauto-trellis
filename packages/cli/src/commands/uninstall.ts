/**
 * `cviauto uninstall` — remove every file written by `cviauto init` / `update`
 * from the current project, plus the `.cviauto/` directory itself.
 *
 * The single source of truth for "what trellis wrote" is
 * `.cviauto/.template-hashes.json`. Files outside that manifest are never
 ...
 * (per the PRD: "全删"). The `.cviauto/` tree is removed unconditionally.
 */
...
interface UninstallPlan {
  ...
  /** Whether `.cviauto/` directory itself will be removed. */
...
/**
 * Execute the plan: write modifications, unlink deletions, remove `.cviauto/`,
...
function executePlan(
  ...
  // has no purpose. `.cviauto` is already gone (step 3), so we skip it.
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  // Pre-check 1: must have a `.cviauto/` directory.
  if (!fs.existsSync(trellisDir)) {
    console.log(
      chalk.gray(
        "Cviauto is not installed in this project (no .cviauto/ directory found).",
      ),
    );
    return;
  }
  ...
  if (Object.keys(hashes).length === 0) {
    console.error(
      chalk.red(
        "Cviauto directory found but manifest is missing — cannot determine which platform files to remove. " +
          "You can manually delete .cviauto/ if needed.",
      ),
    );
    process.exit(1);
  }
  ...
  // manifest only to delete the whole .cviauto/ dir anyway — but the
  ...
  if (pruned.length > 0) {
    ...
    console.log(
      chalk.gray(
        `   Pruned ${pruned.length} orphan manifest entries (user-owned files Cviauto did not write).`,
      ),
    );
  }
  ...
  if (!options.yes) {
    ...
    // honor the same UX as `cviauto update` (which also fails closed in
    // that case).
  ...
  console.log(
    chalk.green(
      `Uninstalled Cviauto: ${summary.deletedFiles} files deleted, ` +
