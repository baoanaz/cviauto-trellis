...
function binaryCheckCommand(
  ...
  return platform === "win32" ? "where cviauto" : "which cviauto";
}
...
export function buildUpgradeCommand(
  ...
  const target = installTarget(tag);
...
function troubleshooting(plan: UpgradeCommandPlan): string {
  return [
    ...
    `- Check which Cviauto binary your shell resolves: ${plan.binaryCheckCommand}`,
    "- If this is a permissions error, fix your Node/npm install or npm prefix; Cviauto does not run sudo.",
    "- If npm reports an existing binary or locked file, resolve that npm error manually; Cviauto does not run --force.",
  ].join("\n");
}
...
export async function upgrade(
  ...
  console.log(chalk.cyan(`Upgrading Cviauto CLI to ${plan.target}`));
  ...
  console.log(chalk.green("\n✓ Cviauto CLI upgrade completed"));
  console.log(chalk.gray("Run: cviauto --version"));
