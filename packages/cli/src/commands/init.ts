...
const CVIAUTO_DEFAULT_SPEC_REGISTRY =
...
export function requireSupportedPython(command: string): string {
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. Cviauto init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but Cviauto init requires Python ≥ 3.9.`,
    );
  }
...
function logPythonAdaptationNotice(command: string): void {
  ...
  console.log(
    chalk.blue(
      `📌 ${osName} detected: Cviauto rendered Python commands as "${command}" in generated hooks, settings, and help text`,
    ),
  );
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    ...
    description:
      "Onboard a new developer to an existing Cviauto project: learn the workflow, conventions, and find assigned work",
    status: "in_progress",
    dev_type: "docs",
    priority: "P1",
    creator: developer,
    ...
    createdAt: today,
    notes:
      "Generated by cviauto init for a new developer joining an existing Cviauto project",
  });
}
...
async function handleReinit(
        ...
        if (platformId) {
          if (configuredPlatforms.has(platformId)) {
            ...
            if (platformId === "claude-code" && options.withStatusline) {
              console.log(
                chalk.gray(
                  "   ↳ Cviauto statusLine installed (--with-statusline)",
                ),
              );
            }
          }
  ...
  if (doAddDeveloper) {
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  ${pythonCmd} .cviauto/scripts/init_developer.py ${devName}`,
        ),
      );
    }
...
interface InitOptions {
  ...
  antigravity?: boolean;
  devin?: boolean;
  /** Deprecated alias for `devin` — kept for programmatic compatibility only. */
  windsurf?: boolean;
  qoder?: boolean;
  codebuddy?: boolean;
  copilot?: boolean;
  ...
  registry?: string;
  monorepo?: boolean;
  /** Claude Code only: install the opt-in Cviauto statusLine (--with-statusline) */
...
function detectDeveloperNameFromGit(cwd: string): string | undefined {
...
async function promptDeveloperName(
  ...
  const answer = await inquirer.prompt<{ developerName?: string }>([
    {
      type: "input",
      name: "developerName",
  ...
  const resolved =
    promptedName && promptedName.length > 0
      ? promptedName
      : (fallbackName ?? "");
  if (!resolved) {
    throw new Error("Developer name is required");
...
export async function init(options: InitOptions): Promise<void> {
  ...
  const developerDefault =
    optionDeveloperName && optionDeveloperName.length > 0
      ? optionDeveloperName
  ...
  let selectedTemplate: string | null = useCviautoDefaultSpecRegistry
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      if (useCviautoDefaultSpecRegistry) {
        console.log(
          chalk.red(
            `Error: Could not install Cviauto default specs from ${CVIAUTO_DEFAULT_SPEC_REGISTRY}.`,
          ),
        );
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: cviauto init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
  ...
  try {
    ...
    for (const tool of tools) {
      ...
      if (platformId) {
        ...
        if (platformId === "claude-code" && options.withStatusline) {
          console.log(
            chalk.gray("   ↳ Cviauto statusLine installed (--with-statusline)"),
          );
        }
      }
    }
  ...
  if (developerName) {
    ...
    if (isFirstInit || tasksEmpty) {
      createBootstrapTask(
        ...
        pythonCmd,
        projectType,
        useCviautoDefaultSpecRegistry ? undefined : monorepoPackages,
        useCviautoDefaultSpecRegistry,
      );
