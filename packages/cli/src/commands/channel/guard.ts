 ...
 *   3. `.cviauto/config.yaml` `channel.worker_guard`
...
/**
 * Parse the `channel.worker_guard` section out of `.cviauto/config.yaml`.
...
export function formatBudgetOverflowError(args: {
  ...
  const hint = [
    ...
    `  cviauto channel kill <channel> --as <worker>`,
    "Or override per spawn:",
    `  cviauto channel spawn ... --max-live-workers ${live.length + 1}`,
    "Or raise the default in .cviauto/config.yaml under channel.worker_guard.max_live_workers.",
  ].join("\n");
