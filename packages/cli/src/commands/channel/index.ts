...
export function registerChannelCommand(program: Command): void {
  ...
  channel
    ...
    .option("--task <path>", "associated Cviauto task directory")
  ...
  channel
    ...
    .option(
      "--agent <agent-name>",
      "load .cviauto/agents/<name>.md (sets default --provider / --model / system prompt)",
    )
    .option(
      "--provider <provider>",
    ...
    .option(
      "--jsonl <path>",
      "parse a Cviauto jsonl manifest ({file, reason} per line) and include each referenced file (repeatable)",
      (val: string, prev: string[] | undefined) => [...(prev ?? []), val],
      [] as string[],
    )
    .option(
  ...
  channel
    ...
    .option(
      "--agent <agent-name>",
      "load .cviauto/agents/<name>.md (sets default --provider / --as / system prompt)",
    )
    .option(
      "--provider <provider>",
      "worker provider: claude | codex (overrides agent)",
    ...
    .option(
      "--jsonl <path>",
      "parse a Cviauto jsonl manifest and include each referenced file (repeatable)",
      (val: string, prev: string[] | undefined) => [...(prev ?? []), val],
      [] as string[],
    )
  ...
  channel
    .command("list")
    .description(
      "List channels in ~/.cviauto/channels/ with worker / activity summary",
    )
    .option("--scope <scope>", "channel scope: project | global")
