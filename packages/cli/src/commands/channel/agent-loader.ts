/**
 * Load a Trellis agent definition from `.cviauto/agents/<name>.md`.
 *
...
export function findAgentFile(name: string, cwd: string): string | null {
  ...
  const agentsRoot = path.resolve(cwd, ".cviauto", "agents");
  const candidates = [
  ...
  if (!file) {
    throw new Error(
      `Agent '${name}' not found. Looked in:\n  ${[
        path.join(cwd, ".cviauto", "agents", `${name}.md`),
        path.join(cwd, ".cviauto", "agents", name, "AGENT.md"),
      ].join("\n  ")}`,
    );
  }
