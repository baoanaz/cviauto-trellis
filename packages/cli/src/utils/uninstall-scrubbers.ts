/**
 * Scrubbers for structured config files during `cviauto uninstall`.
 *
...
/**
 * Cviauto-specific values written by the Pi configurator.
 *
 * The `extensions`/`skills`/`prompts` arrays are paths relative to `.pi/`. We
 * remove the exact entries that the Pi configurator emits.
 */
const PI_CVIAUTO_EXTENSIONS = [
  "./extensions/cviauto/index.ts",
  "./extensions/trellis/index.ts",
];
const PI_CVIAUTO_SKILLS = ["./skills"];
const PI_CVIAUTO_PROMPTS = ["./prompts"];
const PI_SUBAGENTS_PACKAGE = "npm:pi-subagents";

function isCviautoPiEntry(value: unknown, targets: readonly string[]): boolean {
  return typeof value === "string" && targets.includes(value);
}

/**
 * Scrub `.pi/settings.json`:
 * - drop `enableSkillCommands` (cviauto-flagged)
 * - remove cviauto entries from `extensions`/`skills`/`prompts` arrays
 * - remove cviauto-managed `packages["npm:pi-subagents"]` isolation override
...
export function scrubPiSettings(content: string): ScrubResult {
  ...
  const arrayCleanups: [string, readonly string[]][] = [
    ["extensions", PI_CVIAUTO_EXTENSIONS],
    ["skills", PI_CVIAUTO_SKILLS],
    ["prompts", PI_CVIAUTO_PROMPTS],
  ];
  for (const [key, targets] of arrayCleanups) {
    const arr = root[key];
    if (!Array.isArray(arr)) continue;
    const filtered = arr.filter((v) => !isCviautoPiEntry(v, targets));
 ...
 *
 * The current cviauto-emitted file has two distinct chunks:
 * 1. The line `project_doc_fallback_filenames = ["AGENTS.md"]`
 * 2. A multi-line comment block with project-scoped Codex defaults and hook notes.
 *
 ...
 *  - any line that is *only* a comment introduced by cviauto (the entire file
 *    as shipped is comments + that one assignment)
 *  - blank lines that surrounded those removals
 *
 * If the user added their own non-cviauto lines, they are preserved as-is.
...
export function scrubCodexConfigToml(content: string): ScrubResult {
  const cviautoCommentMarkers = [
    "Project-scoped Codex defaults for Cviauto workflows.",
    ...
    "NOTE: Cviauto intentionally does NOT write a [features.multi_agent_v2]",
    ...
    "Without this flag, hooks.json is ignored and Cviauto context won't",
  ...
  // A comment line is "cviauto-known" if its content (after `#` and spaces)
  // matches one of the known marker strings exactly OR is an empty `#` line.
  function isCviautoCommentLine(line: string): boolean {
    const trimmed = line.trim();
    if (!trimmed.startsWith("#")) return false;
    const inner = trimmed.replace(/^#+\s?/, "").trim();
    if (inner.length === 0) return true; // bare `#` line inside cviauto block
    return cviautoCommentMarkers.some((m) => inner === m);
  }

  function isCviautoAssignment(line: string): boolean {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    if (isCviautoAssignment(rawLine) || isCviautoCommentLine(rawLine)) {
      continue; // drop
    }
