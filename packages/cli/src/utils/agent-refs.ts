 ...
 * `cviauto channel spawn --agent <name>`, which loads `.cviauto/agents/<name>.md`
 ...
 * runtime. We surface that mismatch eagerly (at `cviauto init --workflow` /
 * `cviauto workflow --template` time) so users can run `cviauto update` before
 * the first spawn.
 *
 ...
 *
 *   1. `--agent <name>` flag on a `cviauto channel spawn ...` command
 *   2. `.cviauto/agents/<name>.md` literal path reference
 *
...
const AGENT_PATH_RE = new RegExp(
  `\\.cviauto/agents/([${SAFE_AGENT_NAME_CHARS}]+)\\.md`,
  "g",
);

/**
 * Extract the set of `.cviauto/agents/<name>.md` agent names that the given
 ...
 * exist under `<cwd>/.cviauto/agents/`.
 */
