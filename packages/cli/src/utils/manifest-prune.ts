 ...
 * `cviauto update` (before migration classification) and `cviauto uninstall`
 * (before plan building). Together they ensure existing poisoned manifests
 * self-correct on the next routine command.
 *
 * Rules:
 *   - `.cviauto/*` entries are ALWAYS kept. `cviauto uninstall` removes
 *     `.cviauto/` wholesale via `fs.rmSync(..., { recursive: true })`, so
 ...
 *   - Root-level `AGENTS.md` is kept only when it still looks Cviauto-managed
 ...
 *     pruned. This matches "files Cviauto actually wrote during init/update".
 */
...
const MANAGED_BLOCK_MARKERS = [
  { start: "<!-- CVIAUTO:START -->", end: "<!-- CVIAUTO:END -->" },
  { start: "<!-- TRELLIS:START -->", end: "<!-- TRELLIS:END -->" },
] as const;
 ...
 *   - root-level AGENTS.md when it still carries Cviauto managed-block markers
...
export function pruneOrphanManifestKeys(
  ...
  for (const [rawKey, value] of Object.entries(hashes)) {
    ...
    // Always preserve .cviauto/ entries — they're for the workflow tree
    // which uninstall removes wholesale and which update needs for
    // modified-file detection.
    if (key.startsWith(".cviauto/") || key === ".cviauto") {
      kept[key] = value;
      continue;
    }
