...
/**
 * Patterns to exclude from hash tracking (only applied to the .cviauto/ walk).
 */
const EXCLUDE_FROM_HASH = [
  ...
  ".cviauto/spec/", // User-customized spec files
 ...
 *
 * `.cviauto/` is still walked recursively (with `EXCLUDE_FROM_HASH`) because
 * uninstall removes `.cviauto/` wholesale via `rm -rf` regardless of manifest
 * content — accuracy there doesn't affect data-loss, only `cviauto update`
...
export function initializeHashes(
  ...
  if (trackedPaths) {
    for (const relativePath of trackedPaths) {
      // `.cviauto/` paths are handled by the walk below — don't double-track.
      if (relativePath.startsWith(".cviauto/") || relativePath === ".cviauto") {
        continue;
      }
  ...
  // .cviauto/ workflow tree: still walked recursively. Accuracy here is for
  // `cviauto update`'s 3-way merge of workflow.md / config.yaml / scripts;
  // uninstall removes .cviauto/ wholesale so it does not matter for the
  // data-loss bug this contract addresses.
  const files = collectFiles(cwd, ".cviauto");
