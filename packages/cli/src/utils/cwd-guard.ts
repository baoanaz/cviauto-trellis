 ...
 *
 * Running `cviauto init` / `cviauto uninstall` in `$HOME` is catastrophic:
 ...
 * Cviauto manages the same `.{platform}/` config dirs and the hash manifest
...
/**
 * Error message printed by both `cviauto init` and `cviauto uninstall` when
 * the homedir guard trips.
 */
export function homedirGuardMessage(commandName: "init" | "uninstall"): string {
  return (
    `✗ Refusing to run \`cviauto ${commandName}\` in your home directory.\n\n` +
    `Cviauto manages platform config dirs like .claude/, .codex/, .opencode/, which\n` +
    ...
    `Run cviauto from your project directory instead. If you really want to run in\n` +
