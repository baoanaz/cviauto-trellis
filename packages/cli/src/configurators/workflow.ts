...
export interface WorkflowOptions {
  ...
  /**
   * Optional override for `.cviauto/workflow.md` content. When omitted the
   ...
   * Caller is still responsible for removing the `.cviauto/workflow.md` hash
 ...
 *
 * This function creates the .cviauto/ directory structure by:
...
export async function createWorkflowStructure(
  ...
  // Trellis runtime files consumed by `cviauto channel spawn --agent <name>`
  ...
  // can switch to a channel-driven workflow at any time via `cviauto workflow
  // --template`.
  ...
  if (packages && packages.length > 0) {
    // Monorepo mode: create per-package spec directories unless a registry
    // already installed the full .cviauto/spec tree.
