...
/**
 * Per-platform configure options threaded from `cviauto init` flags.
 * Defined here (not in index.ts) so configurators can reference it without
 * a circular import.
 */
export interface PlatformConfigureOptions {
  /**
   * Claude Code only: install the opt-in Cviauto statusLine
   * (`cviauto init --with-statusline`). Off by default — see
 ...
 * rendered in a platform-neutral form (`` `name` (Cviauto command) ``)
 ...
 * of which Cviauto configurator wrote them — eliminating the
 ...
 * neutral. Codex-only skill files (e.g. `cviauto-continue/SKILL.md`,
 * `cviauto-finish-work/SKILL.md` written via `resolveAllAsSkillsNeutral`) DO
...
export function resolvePlaceholdersNeutral(
  ...
  result = result.replace(
    RE_CMD_REF,
    (_match, name: string) => `\`${name}\` (Cviauto command)`,
  );
  result = result.replace(RE_EXECUTOR_AI, context.executorAI);
  result = result.replace(RE_USER_ACTION_LABEL, context.userActionLabel);
  result = result.replace(RE_CLI_FLAG, context.cliFlag);
...
const SKILL_DESCRIPTIONS: Record<string, string> = {
  start:
    "Initializes an AI development session by reading workflow guides, developer identity, git status, active tasks, and project guidelines from .cviauto/. Classifies incoming tasks and routes to brainstorm, direct edit, or task workflow. Use when beginning a new coding session, resuming work, starting a new task, or re-establishing project context.",
  continue:
  ...
  "before-dev":
    "Discovers and injects project-specific coding guidelines from .cviauto/spec/ before implementation begins. Reads spec indexes, pre-development checklists, and shared thinking guides for the target package. Use when starting a new coding task, before writing any code, switching to a different package, or needing to refresh project conventions and standards.",
  brainstorm:
  ...
  "update-spec":
    "Captures executable contracts and coding conventions into .cviauto/spec/ documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions.",
};
...
export function wrapWithSkillFrontmatter(
  ...
  // Look up description by base name (without generated skill prefix)
  const baseName = name.replace(/^(?:trellis|cviauto)-/, "");
...
const COMMAND_DESCRIPTIONS: Record<string, string> = {
  start: "Initialize a Cviauto development session.",
...
import {
  toCviautoGeneratedContent,
  toCviautoGeneratedName,
...
export interface ResolvedSkillFile {
  /** POSIX path relative to the skills root, e.g. "cviauto-meta/SKILL.md" */
  relativePath: string;
  content: string;
}
 ...
 * user-invocable `cviauto-start` skill / `start.md` command as fallback.
...
/**
 * Resolve ALL templates as skills with cviauto- prefix.
...
export function resolveAllAsSkills(ctx: TemplateContext): ResolvedTemplate[] {
  ...
  return templates.map((tmpl) => ({
    name: `cviauto-${tmpl.name}`,
    content: wrapWithSkillFrontmatter(
      `cviauto-${tmpl.name}`,
      toCviautoGeneratedContent(resolvePlaceholders(tmpl.content, ctx)),
    ),
  }));
}
...
export function resolveCommands(ctx: TemplateContext): ResolvedTemplate[] {
  return filterCommands(getCommandTemplates(), ctx).map((tmpl) => ({
    name: tmpl.name,
    content: toCviautoGeneratedContent(resolvePlaceholders(tmpl.content, ctx)),
  }));
}

/**
 * Resolve the auto-triggered skill templates from `common/skills/` with cviauto- prefix + SKILL.md frontmatter.
...
export function resolveSkillsNeutral(ctx: TemplateContext): ResolvedTemplate[] {
  return getSkillTemplates().map((tmpl) => ({
    ...
    content: wrapWithSkillFrontmatter(
      `cviauto-${tmpl.name}`,
      toCviautoGeneratedContent(resolvePlaceholdersNeutral(tmpl.content, ctx)),
    ),
  }));
}
...
export function resolveBundledSkills(
  ...
  return getBundledSkillTemplates().flatMap((skill) =>
    skill.files.map((file) => ({
      relativePath: `${toCviautoGeneratedName(skill.name)}/${file.relativePath}`,
      content: toCviautoGeneratedContent(
...
/** Build the standard "load Cviauto context first" prelude block. */
export function buildPullBasedPrelude(agentType: SubAgentType): string {
  ...
  return replacePythonCommandLiterals(`## Required: Load Cviauto Context First
...
1. **Look at the dispatch prompt** you received from the main agent. If its first line is \`Active task: <path>\` (e.g. \`Active task: .cviauto/tasks/04-17-foo\`), use that path. The main agent is required to include this line on class-2 platforms.
2. **Run** \`python3 ./.cviauto/scripts/task.py current --source\` and read the \`Current task:\` line.
...
/** Best-effort detect agent type from filename ("cviauto-implement.md" → "implement").
...
export function detectSubAgentType(name: string): SubAgentType | null {
  ...
  if (
    base === "cviauto-implement" ||
    base === "cviauto-check" ||
    base === "trellis-implement" ||
    base === "trellis-check"
  ) {
    return base.endsWith("-implement") ? "implement" : "check";
  }
  return null;
}
...
function mapLegacyToolToCopilot(tool: string): string[] {
  switch (tool) {
    ...
    case "Bash":
      return ["execute"];
    // Generic MCP wildcard — used by cviauto-research to opt into "any MCP
