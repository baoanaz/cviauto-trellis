...
export function collectPiTemplates(): Map<string, string> {
  ...
  for (const command of resolvePiCommands()) {
    files.set(`.pi/prompts/cviauto-${command.name}.md`, command.content);
  }
  ...
  files.set(".pi/extensions/cviauto/index.ts", getExtensionTemplate());
...
export async function configurePi(cwd: string): Promise<void> {
  ...
  for (const command of resolvePiCommands()) {
    await writeFile(
      path.join(configRoot, "prompts", `cviauto-${command.name}.md`),
      command.content,
    );
  }
  ...
  ensureDir(path.join(configRoot, "extensions", "cviauto"));
  await writeFile(
    path.join(configRoot, "extensions", "cviauto", "index.ts"),
