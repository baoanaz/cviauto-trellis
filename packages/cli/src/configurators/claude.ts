...
import { getAllAgents, getStatuslineHook } from "../templates/claude/index.js";
 ...
 * Byte-parity matters: `cviauto update` re-derives the expected settings.json
 ...
 * - commands/cviauto/ — start + finish-work as slash commands
 * - skills/cviauto-{name}/SKILL.md — auto-triggered skills from `common/skills/`
 * - with `withStatusline`: opt-in statusline.py hook + `statusLine` settings
 *   entry (off by default; `cviauto init --with-statusline`)
 */
export async function configureClaude(
  ...
  // Copy platform-specific settings — agents are written through the template
  // reader so generated Cviauto names/content match collectTemplates().
  ...
  await writeAgents(path.join(destPath, "agents"), getAllAgents());
  ...
  // not in collectTemplates, so `cviauto update` never force-installs it)
  if (withStatusline) {
    await writeFile(
  ...
  const commandsDir = path.join(destPath, "commands", "cviauto");
