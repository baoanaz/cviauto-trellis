...
export async function configureCodex(cwd: string): Promise<void> {
  ...
  // load Cviauto context themselves via the prelude injected here.
  ...
  // it the workflow breadcrumb won't auto-inject (the cviauto-bootstrap
  ...
  if (!process.env.VITEST && !process.env.TRELLIS_QUIET) {
    process.stderr.write(
      "⚠️  Codex hooks require `features.hooks = true` in your " +
        ...
        "On Codex 0.129+ also run `/hooks` once to approve the Cviauto " +
        "UserPromptSubmit hook. Without these the Cviauto workflow breadcrumb " +
        "won't auto-inject.\n",
    );
  }
