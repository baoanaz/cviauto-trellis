  ...
  function walk(relDir: string): void {
    ...
    for (const entry of readdirSync(absDir)) {
      ...
      if (stat.isDirectory()) {
        ...
        let targetRelEntry = relEntry;
        if (relDir === "agents" && entry.startsWith("trellis-")) {
          targetRelEntry = path.join(
            relDir,
            `${toCviautoGeneratedName(entry.replace(/\.md$/, ""))}.md`,
          );
          content = toCviautoGeneratedContent(content);
        }
        // Map keys are logical paths used as cross-platform hash keys / lookup
        // keys downstream. Always POSIX, regardless of host OS.
        files.set(
          toPosix(path.join(".opencode", targetRelEntry)),
...
/**
 * Collect all opencode template files that `cviauto update` should track.
 *
...
export function collectOpenCodeTemplates(): Map<string, string> {
  ...
  for (const cmd of resolveCommands(ctx)) {
    files.set(`.opencode/commands/cviauto/${cmd.name}.md`, cmd.content);
  }
