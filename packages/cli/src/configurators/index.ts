...
const PLATFORM_FUNCTIONS: Record<AITool, PlatformFunctions> = {
  "claude-code": {
    ...
    collectTemplates: () => {
      ...
      const files = collectBothTemplates(
        ctx,
        (n) => `.claude/commands/cviauto/${n}.md`,
        ".claude/skills",
      );
  ...
  cursor: {
    ...
    collectTemplates: () => {
      const files = collectBothTemplates(
        ...
        (n) => `.cursor/commands/cviauto-${n}.md`,
        ".cursor/skills",
      );
  ...
  gemini: {
    ...
    collectTemplates: () => {
      ...
      const files = new Map<string, string>();
      for (const cmd of resolveCommands(ctx)) {
        const toml = `description = "Cviauto: ${cmd.name}"\n\nprompt = """\n${cmd.content}\n"""\n`;
        files.set(`.gemini/commands/cviauto/${cmd.name}.toml`, toml);
      }
  ...
  devin: {
    ...
    collectTemplates: () =>
      collectBothTemplates(
        ...
        (n) => `.devin/workflows/cviauto-${n}.md`,
        ".devin/skills",
      ),
  },
  qoder: {
  ...
  codebuddy: {
    ...
    collectTemplates: () => {
      const files = collectBothTemplates(
        ...
        (n) => `.codebuddy/commands/cviauto/${n}.md`,
        ".codebuddy/skills",
      );
  ...
  copilot: {
    ...
    collectTemplates: () => {
      ...
      files.set(".github/hooks/cviauto.json", hooksConfig);
      return files;
    },
  },
  droid: {
    ...
    collectTemplates: () => {
      const files = collectBothTemplates(
        ...
        (n) => `.factory/commands/cviauto/${n}.md`,
        ".factory/skills",
      );
...
/**
 * Platforms exposed by the Cviauto init experience.
 *
 * The broader registry remains intact for now so legacy source, update /
 * uninstall internals, and existing tests keep compiling. `cviauto init` uses
 * this allow-list for prompts and `-y` defaults so users only see the
 * company-supported platforms.
 */
export const PUBLIC_INIT_PLATFORM_IDS: readonly AITool[] = [
  "claude-code",
  "codex",
  "opencode",
];

const PUBLIC_INIT_TOOL_NAMES: Partial<Record<AITool, string>> = {
...
/** All directories managed by Cviauto (including .cviauto itself) */
export const ALL_MANAGED_DIRS = [".cviauto", ...new Set(PLATFORM_MANAGED_DIRS)];
...
/**
 * Build the user-facing tool choices for `cviauto init`.
 * Internal platform support can remain in the registry without appearing in
 * the company fork's init prompt or `-y` default path.
 */
export function getPublicInitToolChoices(): {
  key: CliFlag;
  name: string;
  defaultChecked: boolean;
  platformId: AITool;
}[] {
  return PUBLIC_INIT_PLATFORM_IDS.map((id) => ({
    key: AI_TOOLS[id].cliFlag,
    name: PUBLIC_INIT_TOOL_NAMES[id] ?? AI_TOOLS[id].name,
    defaultChecked: AI_TOOLS[id].defaultChecked,
    platformId: id,
  }));
}
