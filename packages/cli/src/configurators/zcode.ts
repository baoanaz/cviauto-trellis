 ...
 * - `.zcode/commands/cviauto/` — slash commands (invoked as /cviauto:<name>)
...
/**
 * Collect all ZCode template files for `cviauto update` diff tracking.
...
export function collectZcodeTemplates(): Map<string, string> {
  ...
  // 2. Commands → .zcode/commands/cviauto/
  for (const cmd of resolveCommands(ctx)) {
    files.set(`.zcode/commands/cviauto/${cmd.name}.md`, cmd.content);
  }
...
export async function configureZcode(cwd: string): Promise<void> {
  ...
  // 2. Commands → .zcode/commands/cviauto/
  const commandsDir = path.join(cwd, ".zcode", "commands", "cviauto");
