 ...
 *   1. Version match between `@baoanaz/cviauto` and
 *      `@baoanaz/cviauto-core` (and the current git tag when checked from
 *      a tag context).
 ...
 *                                    on @baoanaz/cviauto-core resolves
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.[v.coreName];
    if (!dep) {
      fail(`packed CLI is missing dependency on ${v.coreName}.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on ${v.coreName}@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins ${v.coreName} to exact ${v.cliVersion}.`,
    );
  } finally {
