...
import { TrellisContext } from "../src/templates/opencode/lib/cviauto-context.js";

afterEach(() => {
  clearManifestCache();
});

type PlatformTemplateId = Parameters<typeof collectPlatformTemplates>[0];

function expectGeneratedTemplate(
  platform: PlatformTemplateId,
  key: string,
): string {
  const templates = collectPlatformTemplates(platform);
  const content = templates?.get(key);
  expect(
    content,
    `${platform}:${key} generated template should exist`,
  ).toBeDefined();
  return content ?? "";
}
...
describe("regression: add_session.py runtime branch context (issue-106)", () => {
  ...
  function writeTrellisScripts(): void {
    const scriptsDir = path.join(tmpDir, ".cviauto", "scripts");
...
`;
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "workspace", "test-dev", "index.md"),
      indexContent,
      "utf-8",
    );
  }
  ...
  function setupSessionRepo(options?: {
    ...
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "workspace", "test-dev"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-22T00:00:00\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "workspace", "test-dev", "journal-1.md"),
    ...
    if (options?.taskBranch || options?.taskBaseBranch) {
      const taskDir = path.join(tmpDir, ".cviauto", "tasks", "issue-106");
      fs.mkdirSync(taskDir, { recursive: true });
      fs.mkdirSync(path.join(tmpDir, ".cviauto", ".runtime", "sessions"), {
        recursive: true,
      });
      fs.writeFileSync(
        path.join(tmpDir, ".cviauto", ".runtime", "sessions", "session-a.json"),
        JSON.stringify(
          {
            current_task: ".cviauto/tasks/issue-106",
            platform: "test",
          },
          null,
          2,
  ...
  function runAddSession(title: string, options?: { branch?: string }): void {
    const command = [
      "python3",
      JSON.stringify(
        path.join(tmpDir, ".cviauto", "scripts", "add_session.py"),
      ),
      "--title",
...
describe("regression: Windows path separator (beta.12)", () => {
  it("[beta.12] isManagedPath handles Windows backslash paths", () => {
    ...
    expect(isManagedPath(".cviauto\\spec\\backend")).toBe(true);
    ...
    expect(isManagedPath(".github\\hooks\\cviauto.json")).toBe(true);
  });
...
describe("regression: task directory paths (0.2.14, 0.2.15, beta.13)", () => {
  it("[0.2.15] PATHS.TASKS is .cviauto/tasks (not .cviauto/workspace/*/tasks)", () => {
    expect(PATHS.TASKS).toBe(".cviauto/tasks");
    expect(PATHS.TASKS).not.toContain("workspace");
  });

  it("[0.2.14] Claude agent templates do not contain hardcoded .cviauto/workspace/*/tasks/ paths", () => {
    const agents = getClaudeAgents();
    for (const agent of agents) {
      expect(agent.content).not.toMatch(/\.cviauto\/workspace\/[^/]+\/tasks\//);
    }
  });
...
describe("regression: resolve_task_dir path handling", () => {
  it("[beta.12] resolve_task_dir handles .cviauto prefix", () => {
    // The function should recognize .cviauto-prefixed paths as relative paths
    expect(commonTaskUtils).toContain('.startswith(".cviauto")');
  });
...
describe("regression: migration data integrity (beta.14)", () => {
  ...
  it("[statusline-opt-in] statusline.py is not in claude's collected templates (update must not force-install it)", () => {
    // The opt-in statusline (`cviauto init --with-statusline`) must stay out
...
describe("regression: update only configured platforms (beta.16)", () => {
  ...
  // `cviauto update` silently ignored .opencode/, so CLI-side changes to
  ...
  it("[beta.8] collectPlatformTemplates returns Map for opencode (plugins + agents + lib + package.json + commands + skills)", () => {
    ...
    // fix was a plugin-shape change that couldn't be delivered via `cviauto update`.
    ...
    expect(result.has(".opencode/agents/cviauto-implement.md")).toBe(true);
    expect(result.has(".opencode/lib/cviauto-context.js")).toBe(true);
...
describe("regression: agent-session Trellis update hint", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "tasks"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", ".developer"),
  ...
  function runContextWithTrellisOutput(
    ...
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", ".version"),
      `${currentVersion}\n`,
      "utf-8",
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "sys.path.insert(0, str(Path.cwd() / '.cviauto' / 'scripts'))",
  ...
  it("shows a concise update hint when trellis --version reports a newer version", () => {
    ...
    expect(output).toContain("run cviauto upgrade");
  ...
  it("does not show a hint when installed version is equal or newer", () => {
    ...
    fs.rmSync(path.join(tmpDir, ".cviauto", ".runtime"), {
      recursive: true,
      force: true,
    });
...
describe("regression: issue #252 polyrepo Git context", () => {
  ...
  function writeConfigYaml(content: string): void {
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "config.yaml"),
      content,
      "utf-8",
    );
  }
...
describe("regression: current-task path normalization", () => {
  ...
  function writeLegacyCurrentTask(taskRef: string): void {
    writeProjectFile(path.join(".cviauto", ".current-task"), `${taskRef}\n`);
  }

  function writeSessionContext(contextKey: string, taskRef: string): void {
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", `${contextKey}.json`),
      JSON.stringify(
        {
  ...
  function setupTaskRepo(): void {
    writeTrellisScripts();
    writeProjectFile(
      path.join(".cviauto", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-27T00:00:00\n",
    );
    writeProjectFile(path.join(".cviauto", "workflow.md"), "# Workflow\n");
    writeProjectFile(
      path.join(".cviauto", "spec", "guides", "index.md"),
      "# Guides\n",
    );
    writeProjectFile(
      path.join(".cviauto", "tasks", "issue-106", "task.json"),
      JSON.stringify(
        {
          title: "Issue 106 task",
          status: "in_progress",
    ...
    writeProjectFile(
      path.join(".cviauto", "tasks", "issue-106", "prd.md"),
      "# PRD\n",
    );
    writeProjectFile(
      path.join(".cviauto", "tasks", "issue-106", "implement.jsonl"),
  ...
  it("[session-current-task] task.py start without context key enters degraded mode (returns 0, no pointer)", () => {
    ...
    // proceed.
    setupTaskRepo();
    const taskScriptPath = path.join(tmpDir, ".cviauto", "scripts", "task.py");

    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".cviauto\\\\tasks\\\\issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv(),
    ...
    expect(fs.existsSync(path.join(tmpDir, ".cviauto", ".current-task"))).toBe(
      false,
    );
    expect(fs.existsSync(path.join(tmpDir, ".cviauto", ".runtime"))).toBe(
      false,
    );
  ...
  it("[session-current-task] task.py start writes session runtime state when TRELLIS_CONTEXT_ID is set", () => {
    ...
    expect(context.current_task).toBe(".cviauto/tasks/issue-106");
    expect(fs.existsSync(path.join(tmpDir, ".cviauto", ".current-task"))).toBe(
      false,
    );
  });
  ...
  it("[session-current-task] task.py finish deletes the session runtime context", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".cviauto/tasks/issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
  ...
  it("[workflow-state-r7] task.py create auto-sets session pointer when TRELLIS_CONTEXT_ID is set (planning breadcrumb reachable)", () => {
    ...
    const taskDir = fs
      .readdirSync(path.join(tmpDir, ".cviauto", "tasks"))
    ...
    expect(context.current_task).toBe(`.cviauto/tasks/${taskDir}`);
  });
  ...
  it("[workflow-state-r7] task.py create degrades silently without session identity (no .runtime side effect)", () => {
    ...
    const sessionsDir = path.join(tmpDir, ".cviauto", ".runtime", "sessions");
  ...
  it("[workflow-state-r7] task.py create then task.py start is idempotent (pointer + status flip)", () => {
    ...
    expect(taskDir).toBeDefined();
    const relTaskDir = path.posix.join(".cviauto", "tasks", taskDir as string);
  ...
  it("[session-current-task] task.py archive deletes runtime sessions pointing at the archived task", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "session-a.json"),
      JSON.stringify({ current_task: ".cviauto/tasks/issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "session-b.json"),
      JSON.stringify({ current_task: "issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "session-other.json"),
      JSON.stringify({ current_task: ".cviauto/tasks/other-task" }, null, 2),
    );
  ...
  it("[task-lifecycle] task.py create refuses an archived task dir-name collision", () => {
    ...
    const tasksDir = path.join(tmpDir, ".cviauto", "tasks");
    ...
    expect(result.stderr).toContain(".cviauto/tasks/archive/");
  ...
  it("[task-input-contract] task.py archive accepts task name, relative path, and absolute path", () => {
    ...
    for (const name of taskNames) {
      writeProjectFile(
        path.join(".cviauto", "tasks", name, "task.json"),
        JSON.stringify(
          {
            title: `Task ${name}`,
            status: "in_progress",
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} archive ${JSON.stringify(`.cviauto/tasks/${taskNames[1]}`)} --no-commit`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv(),
    ...
    const absPath = path.join(tmpDir, ".cviauto", "tasks", taskNames[2]);
    execSync(
    ...
    for (const name of taskNames) {
      expect(
        fs.existsSync(path.join(tmpDir, ".cviauto", "tasks", name)),
        `task ${name} should no longer exist in active tasks/`,
      ).toBe(false);
    }

    // All three should appear under archive/<YYYY-MM>/.
    const archiveRoot = path.join(tmpDir, ".cviauto", "tasks", "archive");
  ...
  it("[session-current-task] task.py finish ignores legacy .current-task when no session task is set", () => {
    setupTaskRepo();
    writeLegacyCurrentTask(".cviauto/tasks/issue-106");
    const taskScriptPath = path.join(tmpDir, ".cviauto", "scripts", "task.py");
  ...
  it("[session-current-task] stale session task does not fall back to legacy .current-task", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "session-b.json"),
      JSON.stringify(
        { current_task: ".cviauto/tasks/missing-task", platform: "test" },
        null,
        2,
      ),
    );
    ...
    expect(output).toContain("Current task: .cviauto/tasks/missing-task");
  ...
  it("[session-current-task] Claude statusline uses session-scoped task when session_id is present", () => {
    ...
    writeLegacyCurrentTask(".cviauto/tasks/issue-106");
    writeProjectFile(
      path.join(".cviauto", "tasks", "session-task", "task.json"),
      JSON.stringify(
        {
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "claude_status-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/session-task",
          platform: "claude",
        },
        null,
        2,
  ...
  it("[session-current-task] Python session-start hooks resolve session backslash refs without stale pointer", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".cviauto\\tasks\\issue-106");
    writeSessionContext("codex_session-a", ".cviauto\\tasks\\issue-106");
  ...
  it("[session-current-task] Cursor beforeShellExecution bridges conversation_id into task.py shell commands", () => {
    ...
    const hookOutput = runPython(
      ...
      JSON.stringify({
        ...
        generation_id: "gen-a",
        cwd: tmpDir,
        command: `${pythonCmd} ./.cviauto/scripts/task.py start .cviauto/tasks/issue-106 && ${pythonCmd} ./.cviauto/scripts/task.py current --source`,
    ...
    expect(currentOutput).toContain("Current task: .cviauto/tasks/issue-106");
  ...
  it("[session-current-task] Cursor preToolUse injects context for custom Task subagents", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "cursor_parent-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/issue-106",
          current_run: null,
    ...
    expect(prompt).toContain(
      "=== .cviauto/tasks/issue-106/prd.md (Requirements) ===",
    );
  ...
  it("[session-current-task] Cursor hook uses conversation_id when transcript_path is null", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", "tasks", "cursor-task", "task.json"),
      JSON.stringify(
        {
          id: "cursor-task",
          title: "Cursor task",
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "cursor_cursor-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/cursor-task",
          platform: "cursor",
        },
        null,
        2,
  ...
  it("[session-current-task] OpenCode resolver ignores legacy .current-task and uses plugin sessionID", () => {
    ...
    writeLegacyCurrentTask(".cviauto/tasks/issue-106");
    writeProjectFile(
      path.join(".cviauto", "tasks", "opencode-task", "task.json"),
      JSON.stringify(
        {
          title: "OpenCode task",
          status: "in_progress",
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "opencode_oc-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/opencode-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    expect(none.taskPath).toBe(".cviauto/tasks/opencode-task");
    ...
    expect(active.taskPath).toBe(".cviauto/tasks/opencode-task");
  ...
  it("[session-current-task] OpenCode resolver prefers OPENCODE_RUN_ID over plugin sessionID", () => {
    setupTaskRepo();
    writeProjectFile(
      path.join(".cviauto", "tasks", "opencode-run-task", "task.json"),
      JSON.stringify(
        {
    ...
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "opencode_run-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/opencode-run-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    try {
      ...
      expect(active.taskPath).toBe(".cviauto/tasks/opencode-run-task");
      expect(active.stale).toBe(false);
    } finally {
  ...
  it("[#240] Codex SessionStart output uses compact context without generic sub-agent notice", () => {
    ...
    expect(ctx).toContain("Cviauto compact SessionStart context");
  ...
  it("[workflow-v2] shared session-start summarizes in-progress context without auto-dispatch approval", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".cviauto/tasks/issue-106");
  ...
  it("[session-current-task] OpenCode context layer normalizes backslash refs for downstream plugins", () => {
    setupTaskRepo();
    writeSessionContext("opencode_oc-a", ".cviauto\\tasks\\issue-106");
    ...
    expect(ctx.getCurrentTask({ sessionID: "oc-a" })).toBe(
      ".cviauto/tasks/issue-106",
    );
    expect(ctx.resolveTaskDir(".cviauto\\tasks\\issue-106")).toBe(
      path.join(tmpDir, ".cviauto", "tasks", "issue-106"),
    );
  });
  ...
  it("[session-fallback] single session file — fallback returns its task with session-fallback source", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_parent", ".cviauto/tasks/issue-106");

    const { output, status } = runTaskCurrent();
    expect(status).toBe(0);
    expect(output).toContain("Current task: .cviauto/tasks/issue-106");
  ...
  it("[session-fallback] multiple session files — refuses to guess, returns none", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_a", ".cviauto/tasks/issue-106");
    writeProjectFile(
      path.join(".cviauto", "tasks", "other-task", "task.json"),
      JSON.stringify({ title: "other", status: "in_progress" }, null, 2),
    );
    writeSessionContext("codex_session_b", ".cviauto/tasks/other-task");
  ...
  it("[session-fallback] explicit context-key match takes precedence over fallback", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_explicit", ".cviauto/tasks/issue-106");
  ...
  function writeWorkflowStateHook(): void {
    writeProjectFile(
      path.join(".cviauto", "hooks", "inject-workflow-state.py"),
  ...
  function writeWorkflowMd(body: string): void {
    writeProjectFile(path.join(".cviauto", "workflow.md"), body);
  }
  ...
  it("[workflow-state] missing/empty workflow.md degrades to generic line (post-R5: no fallback dict)", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".cviauto/tasks/issue-106");
    writeWorkflowStateHook();
    ...
    expect(parsed.hookSpecificOutput.additionalContext).not.toContain(
      "cviauto-implement → cviauto-check",
    );
  });
  ...
  it("[workflow-state] in_progress tag in workflow.md mentions Phase 3.4 commit (R1 invariant)", () => {
    ...
    writeWorkflowMd(
      "[workflow-state:in_progress]\n" +
        "Flow: cviauto-implement → cviauto-check → commit (Phase 3.4) → /cviauto:finish-work. Run cviauto-update-spec only on explicit spec-promotion request.\n" +
        "[/workflow-state:in_progress]\n",
    );
    ...
    expect(parsed.hookSpecificOutput.additionalContext).toContain(
      "explicit spec-promotion request",
    );
  });
  ...
  it("[workflow-state] CWD drift: hook finds .cviauto/ when invoked from subdirectory", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".cviauto/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] no_task breadcrumb emitted when no session active task exists", () => {
    writeTrellisScripts();
    writeProjectFile(path.join(".cviauto", ".developer"), "name=test\n");
    ...
    // routing to cviauto-brainstorm content surfaces.
    writeProjectFile(
      path.join(".cviauto", "workflow.md"),
      "[workflow-state:no_task]\n" +
        "No active task. Load `cviauto-brainstorm` skill to start.\n" +
        "[/workflow-state:no_task]\n",
    );
    writeLegacyCurrentTask(".cviauto/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] silent exit 0 when not a Trellis project (no .cviauto/ dir)", () => {
    // No .cviauto/ at all — hook should silently exit
    writeWorkflowStateHook();
    fs.rmSync(path.join(tmpDir, ".cviauto"), { recursive: true, force: true });
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "hooks"), { recursive: true });
    fs.copyFileSync(
      path.join(
        __dirname,
        "..",
      ...
      path.join(tmpDir, ".cviauto", "hooks", "inject-workflow-state.py"),
    );
    // Now .cviauto/ exists only as a parent for the hook script — need to move
    // the hook out of .cviauto/ so root-finding fails. Use a fully separate dir.
  ...
  it("[init-context-removal] task.py init-context is deprecated with clear pointer to planning artifacts", () => {
    ...
    try {
      execSync(
        `${pythonCmd} ${JSON.stringify(taskScriptPath)} init-context .cviauto/tasks/issue-106 fullstack`,
        { cwd: tmpDir, encoding: "utf-8" },
      );
    } catch (err) {
      threw = true;
...
`;
  ...
  it("[issue-237] all implement/check agent templates contain recursion guards", () => {
    const agentFiles: {
    ...
    }[] = [
      {
        ...
        key: ".claude/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "claude-code",
        key: ".claude/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "codebuddy",
        key: ".codebuddy/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "codebuddy",
        key: ".codebuddy/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "codex",
        key: ".codex/agents/cviauto-implement.toml",
        agent: "implement",
      },
      {
        platform: "codex",
        key: ".codex/agents/cviauto-check.toml",
        agent: "check",
      },
      {
        platform: "cursor",
        key: ".cursor/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "cursor",
        key: ".cursor/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "gemini",
        key: ".gemini/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "gemini",
        key: ".gemini/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "kiro",
        key: ".kiro/agents/cviauto-implement.json",
        agent: "implement",
      },
      {
        platform: "kiro",
        key: ".kiro/agents/cviauto-check.json",
        agent: "check",
      },
      {
        platform: "opencode",
        key: ".opencode/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "opencode",
        key: ".opencode/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "pi",
        key: ".pi/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "pi",
        key: ".pi/agents/cviauto-check.md",
        agent: "check",
      },
      {
        platform: "qoder",
        key: ".qoder/agents/cviauto-implement.md",
        agent: "implement",
      },
      {
        platform: "qoder",
        key: ".qoder/agents/cviauto-check.md",
        agent: "check",
      },
    ];
  ...
  it("[issue-241-followup] codex sub-agent toml files disable collab tools at protocol level", () => {
    ...
    const codexAgentFiles = [
      ".codex/agents/cviauto-implement.toml",
      ".codex/agents/cviauto-check.toml",
      ".codex/agents/cviauto-research.toml",
    ];

    for (const relativePath of codexAgentFiles) {
      const content = expectGeneratedTemplate("codex", relativePath);
      expect(
        content,
  ...
  it("[workflow-v2] --mode phase --platform codex (sub-agent mode) filters out generic before-dev routing", () => {
    ...
    // explicitly so the legacy spawn-cviauto-implement block surfaces.
    ...
    expect(output).toContain("cviauto-implement");
    expect(output).not.toContain(
      "| About to write code / start implementing | cviauto-before-dev |",
    );
  ...
  it("[workflow-v2] --mode phase --platform kilo keeps cviauto-before-dev routing (agent-less path)", () => {
    ...
    // see `cviauto-before-dev` because they write code in the main session.
    writeTrellisScripts();
    writeProjectFile(path.join(".cviauto", ".developer"), "name=test\n");
    writeProjectFile(
      path.join(".cviauto", "workflow.md"),
    ...
    expect(output).not.toContain("Dispatch the `cviauto-implement` sub-agent");
  });

  // ------------------------------------------------------------
  // session-start.py <cviauto-workflow> + <guidelines> compact context
  // ------------------------------------------------------------

  it("[workflow-v2] session-start.py <cviauto-workflow> block contains compact Phase Index", () => {
    writeTrellisScripts();
    writeProjectFile(path.join(".cviauto", ".developer"), "name=test\n");
    writeProjectFile(
      path.join(".cviauto", "workflow.md"),
    ...
    const workflowMatch =
      /<cviauto-workflow>([\s\S]*?)<\/cviauto-workflow>/.exec(ctx);
  ...
  it("[workflow-v2] session-start.py <guidelines> block lists context order and spec paths", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", "spec", "cli", "backend", "index.md"),
    ...
    expect(guidelinesBlock).toContain(".cviauto/spec/cli/backend/index.md");
  ...
  it("[workflow-v2] inject-subagent-context.py does NOT write current_phase when implement spawns", () => {
    ...
    writeTrellisScripts();
    writeProjectFile(path.join(".cviauto", ".developer"), "name=test\n");
    writeProjectFile(path.join(".cviauto", "workflow.md"), "# Minimal\n");
    // Session active task WITHOUT current_phase field (post-migration state)
    writeProjectFile(
      path.join(".cviauto", ".runtime", "sessions", "claude_phase-a.json"),
      JSON.stringify(
        {
          current_task: ".cviauto/tasks/issue-106",
          platform: "claude",
    ...
    const input = JSON.stringify({
      tool_name: "Task",
      tool_input: {
        subagent_type: "cviauto-implement",
        prompt: "do work",
      },
      cwd: tmpDir,
      session_id: "phase-a",
    ...
    const taskJson = JSON.parse(
      fs.readFileSync(
        path.join(tmpDir, ".cviauto", "tasks", "issue-106", "task.json"),
        "utf-8",
      ),
  ...
  function writeConfigYaml(content: string): void {
    writeProjectFile(path.join(".cviauto", "config.yaml"), content);
  }
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb defaults to inline dispatch when config absent", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", "workflow.md"),
      "[workflow-state:in_progress]\n" +
        "DISPATCH the cviauto-implement / cviauto-check sub-agents.\n" +
        "[/workflow-state:in_progress]\n" +
        "[workflow-state:in_progress-inline]\n" +
        "MAIN SESSION edits code via cviauto-before-dev directly.\n" +
        "[/workflow-state:in_progress-inline]\n",
    );
    ...
    expect(ctx).not.toContain("DISPATCH the cviauto-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to plain status when codex.dispatch_mode=sub-agent", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("DISPATCH the cviauto-implement");
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to inline tag when codex.dispatch_mode=inline", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("MAIN SESSION edits code");
    expect(ctx).toContain("cviauto-before-dev");
    expect(ctx).not.toContain("DISPATCH the cviauto-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] inline `#` comment after value is stripped (config.yaml uncomment leaves trailing hint)", () => {
    ...
    writeTrellisScripts();
    writeProjectFile(
      path.join(".cviauto", "hooks", "cviauto_config.py"),
      expectTemplateContent(
        getAllScripts().get("common/cviauto_config.py") ?? "",
        "cviauto_config",
      ),
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "from pathlib import Path",
        `hook_path = Path(${JSON.stringify(
          path.join(tmpDir, ".cviauto", "hooks", "cviauto_config.py"),
        )})`,
  ...
  it("[issue-codex-dispatch-mode] resolve_effective_platform namespaces codex into codex-sub-agent / codex-inline", () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import sys, json",
        `sys.path.insert(0, ${JSON.stringify(path.join(tmpDir, ".cviauto", "scripts"))})`,
  ...
  it("[issue-codex-dispatch-mode] codex hook injects <codex-mode> banner reflecting dispatch_mode", () => {
    ...
    writeProjectFile(
      path.join(".cviauto", "workflow.md"),
      "[workflow-state:in_progress]\nDISPATCH the cviauto-implement.\n[/workflow-state:in_progress]\n[workflow-state:in_progress-inline]\nMAIN SESSION inline edit.\n[/workflow-state:in_progress-inline]\n",
    );
...
describe("regression: cli_adapter platform support (beta.9, beta.13, beta.16)", () => {
  ...
  it("[pi] cli_adapter.py supports pi platform", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".pi/prompts/cviauto-{name}.md"',
    );
  });
  ...
  it("[droid] cli_adapter.py treats droid as commands-only (no CLI run/resume yet)", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".factory/commands/cviauto/{name}.md"',
    );
  });

  it("[droid] cli_adapter.py has explicit droid branches in all key methods", () => {
    expect(commonCliAdapter).toMatch(
      /def get_trellis_command_path[\s\S]*?elif self\.platform == "droid":[\s\S]*?\.factory\/commands\/cviauto\//,
    );
    expect(commonCliAdapter).toMatch(
  ...
  it("[migrate-flow-bugs] get_trellis_command_path codex branch uses trellis- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_trellis_command_path[\s\S]*?elif self\.platform == "codex":[\s\S]*?return f"\.agents\/skills\/cviauto-\{name\}\/SKILL\.md"/,
    );
  ...
  it("[migrate-flow-bugs] get_trellis_command_path kiro branch uses trellis- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_trellis_command_path[\s\S]*?elif self\.platform == "kiro":[\s\S]*?return f"\.kiro\/skills\/cviauto-\{name\}\/SKILL\.md"/,
    );
    expect(commonCliAdapter).not.toMatch(
  ...
  it("[migrate-flow-bugs] detect_platform has codex shared-skills fallback guarded by no-other-platform-dir check", () => {
    // Fallback fires when .agents/skills/cviauto-* exists AND no other
  ...
  it("[migrate-flow-bugs] 0.5.0-beta.0 manifest has migrationGuide + aiInstructions (back-filled)", () => {
    ...
    expect(manifest.migrationGuide).toMatch(/cviauto-/); // skill renames
...
describe("regression: migration manifest consistency", () => {
  ...
  it("[#57] shell archive migrations use rename type with correct from/to paths", () => {
    ...
    for (const m of shellArchives) {
      ...
      expect(m.from).toMatch(/\.cviauto\/scripts\/.*\.sh$/);
      expect(m.to).toMatch(/\.cviauto\/scripts-shell-archive\/.*\.sh$/);
  ...
  describe("[0.5.0-beta.0] command→skill rename coverage", () => {
    ...
    const PLATFORMS: { id: string; from: PathFn; to: PathFn }[] = [
      {
        id: "claude",
        from: (n) => `.claude/commands/cviauto/${n}.md`,
        to: (n) => `.claude/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "cursor",
        from: (n) => `.cursor/commands/cviauto-${n}.md`,
        to: (n) => `.cursor/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "opencode",
        from: (n) => `.opencode/commands/cviauto/${n}.md`,
        to: (n) => `.opencode/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "codebuddy",
        from: (n) => `.codebuddy/commands/cviauto/${n}.md`,
        to: (n) => `.codebuddy/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "droid",
        from: (n) => `.factory/commands/cviauto/${n}.md`,
        to: (n) => `.factory/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "gemini",
        from: (n) => `.gemini/commands/cviauto/${n}.toml`,
        to: (n) => `.gemini/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "copilot",
        from: (n) => `.github/prompts/${n}.prompt.md`,
        to: (n) => `.github/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "kilo",
        from: (n) => `.kilocode/workflows/${n}.md`,
        to: (n) => `.kilocode/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "antigravity",
        from: (n) => `.agent/workflows/${n}.md`,
        to: (n) => `.agent/skills/cviauto-${n}/SKILL.md`,
      },
      {
        ...
        from: (n) => `.windsurf/workflows/cviauto-${n}.md`,
        to: (n) => `.windsurf/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "kiro",
        from: (n) => `.kiro/skills/${n}/SKILL.md`,
        to: (n) => `.kiro/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "qoder",
        from: (n) => `.qoder/skills/${n}/SKILL.md`,
        to: (n) => `.qoder/skills/cviauto-${n}/SKILL.md`,
      },
      {
        id: "shared",
        from: (n) => `.agents/skills/${n}/SKILL.md`,
        to: (n) => `.agents/skills/cviauto-${n}/SKILL.md`,
      },
    ];
    ...
    it("breaking + recommendMigrate flags are both set (drives gate)", () => {
      ...
      // by running `cviauto update` without `--migrate`.
...
describe("regression: collectTemplates paths match init directory structure (0.3.1)", () => {
  ...
  it("[devin] devin uses workflows/ instead of commands/cviauto/", () => {
  ...
  it("[copilot] collectTemplates tracks hooks and VS Code discovery config", () => {
    ...
    expect(keys).toContain(".github/hooks/cviauto.json");
  });
});
...
describe("regression: parse_simple_yaml uses _unquote not greedy strip (0.3.8)", () => {
  ...
  it("config.py uses _unquote for key-value, not .strip('\"')", () => {
    ...
    // mirrors cviauto_config.py so YAML `key: false  # comment` parses
  ...
  const class2 = [
    {
      ...
      preludeAgents: [
        ".qoder/agents/cviauto-implement.md",
        ".qoder/agents/cviauto-check.md",
      ],
      nonPreludeAgents: [".qoder/agents/cviauto-research.md"],
    },
    {
      id: "gemini" as const,
      hooksDir: ".gemini/hooks",
      preludeAgents: [
        ".gemini/agents/cviauto-implement.md",
        ".gemini/agents/cviauto-check.md",
      ],
      nonPreludeAgents: [".gemini/agents/cviauto-research.md"],
    },
    {
      id: "codex" as const,
      ...
      nonPreludeAgents: [".codex/agents/cviauto-research.toml"],
    },
    {
      id: "copilot" as const,
      hooksDir: ".github/copilot/hooks",
      preludeAgents: [
        ".github/agents/cviauto-implement.agent.md",
        ".github/agents/cviauto-check.agent.md",
      ],
      nonPreludeAgents: [".github/agents/cviauto-research.agent.md"],
    },
  ];
    ...
    describe(`[${id}]`, () => {
      ...
      it("implement/check definitions contain pull-based prelude", () => {
        for (const file of preludeAgents) {
          ...
          expect(content).toContain("Required: Load Cviauto Context First");
      ...
      it("[beta.21] prelude is injected exactly once, not duplicated", () => {
        ...
        for (const file of preludeAgents) {
          const content = fs.readFileSync(path.join(tmpDir, file), "utf-8");
          const occurrences =
            content.split("Required: Load Cviauto Context First").length - 1;
      ...
      it("research definition does NOT contain pull-based prelude", () => {
        // research is orthogonal: it searches .cviauto/spec/ and doesn't
        ...
        for (const file of nonPreludeAgents) {
          const content = fs.readFileSync(path.join(tmpDir, file), "utf-8");
          expect(content).not.toContain("Required: Load Cviauto Context First");
        }
      });
...
describe("regression: copilot agents use YAML tools frontmatter", () => {
  ...
  it("writes Copilot agent tools as YAML lists", () => {
    ...
    // equivalents.
    const content = fs.readFileSync(
      path.join(tmpDir, ".github/agents/cviauto-implement.agent.md"),
      "utf-8",
    );
  ...
  it("maps research agent MCP tools to Copilot tool names", () => {
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, ".github/agents/cviauto-research.agent.md"),
      "utf-8",
    );
    const frontmatter = content.split("---\n")[1] ?? "";
  ...
  it("collectPlatformTemplates matches written Copilot agent output", () => {
    ...
    const generated = fs.readFileSync(
      path.join(tmpDir, ".github/agents/cviauto-check.agent.md"),
      "utf-8",
    );
    expect(templates?.get(".github/agents/cviauto-check.agent.md")).toBe(
      generated,
    );
  });
});
...
describe("regression: pi uses TypeScript extension assets instead of Python hooks", () => {
  ...
  it("installs a subagent-capable extension and pull-based agent context", () => {
    const extension = fs.readFileSync(
      path.join(tmpDir, ".pi", "extensions", "cviauto", "index.ts"),
      "utf-8",
    );
    expect(extension).toContain('name: "cviauto_subagent"');
    expect(extension).toContain('pi.on?.("before_agent_start"');
    expect(extension).toContain('pi.on?.("tool_call"');

    for (const agent of ["cviauto-implement.md", "cviauto-check.md"]) {
      const content = fs.readFileSync(
...
describe("regression: research agent persists findings to task dir", () => {
  ...
  const markdownPlatforms: {
    ...
    { platform: "claude-code", key: ".claude/agents/cviauto-research.md" },
    { platform: "cursor", key: ".cursor/agents/cviauto-research.md" },
    { platform: "qoder", key: ".qoder/agents/cviauto-research.md" },
    { platform: "codebuddy", key: ".codebuddy/agents/cviauto-research.md" },
    { platform: "droid", key: ".factory/droids/cviauto-research.md" },
  ];

  for (const { platform, key } of markdownPlatforms) {
    it(`[${platform}:${key}] has Write tool and persist instruction`, () => {
      const content = expectGeneratedTemplate(platform, key);
  ...
  it("[gemini:.gemini/agents/cviauto-research.md] omits tools line + has persist instruction", () => {
    const content = expectGeneratedTemplate(
      "gemini",
      ".gemini/agents/cviauto-research.md",
    );
    const fm = content.split("---\n")[1] ?? "";
  ...
  it("kiro research.json includes write tool and persist instruction", () => {
    const content = expectGeneratedTemplate(
      "kiro",
      ".kiro/agents/cviauto-research.json",
    );
  ...
  it("opencode research.md grants write/edit permission and has persist instruction", () => {
    const content = expectGeneratedTemplate(
      "opencode",
      ".opencode/agents/cviauto-research.md",
    );
    const fm = content.split("---\n")[1] ?? "";
...
describe("regression: templates/markdown/spec contains only .md.txt files (0.5.0-beta.9)", () => {
  ...
  // .cviauto/spec/cli/backend/directory-structure.md "Don't: Leak dogfood
...
describe("regression: Gemini CLI 0.40.x template compatibility (#224)", () => {
  ...
  it("[#224] codex + gemini render byte-identical content for shared `.agents/skills/` files", () => {
    ...
    // At least the shared common skills + bundled cviauto-meta files must
  ...
  it("[#224] needsCodexUpgrade looks for command-as-skill markers, not bare `.agents/skills/` prefix", () => {
    ...
    // (`cviauto-continue/SKILL.md`, `cviauto-finish-work/SKILL.md`) and the
    ...
    expect(updateSrc).toMatch(/\.agents\/skills\/cviauto-continue\/SKILL\.md/);
...
describe("regression: sub-agent context injection fallback (0.5.3)", () => {
  ...
  // marker, and every class-1 cviauto-implement / cviauto-check definition
  ...
  const CLASS1_MD_AGENT_FILES: {
    ...
    {
      platform: "droid",
      key: ".factory/droids/cviauto-implement.md",
      agent: "implement",
    },
    {
      platform: "droid",
      key: ".factory/droids/cviauto-check.md",
      agent: "check",
    },
  ];
  ...
  for (const { platform, key, agent } of CLASS1_MD_AGENT_FILES) {
  ...
  for (const agent of ["implement", "check"] as const) {
    it(`kiro/${agent} JSON agent carries marker + fallback protocol in prompt`, () => {
      ...
      const content = expectGeneratedTemplate(
        "kiro",
        `.kiro/agents/cviauto-${agent}.json`,
      );
  ...
  const GEMINI_QODER_AGENT_FILES: {
    ...
    { platform: "gemini", key: ".gemini/agents/cviauto-implement.md" },
    { platform: "gemini", key: ".gemini/agents/cviauto-check.md" },
    { platform: "qoder", key: ".qoder/agents/cviauto-implement.md" },
    { platform: "qoder", key: ".qoder/agents/cviauto-check.md" },
  ];

  for (const { platform, key } of GEMINI_QODER_AGENT_FILES) {
    it(`${platform}:${key} references task artifacts`, () => {
      const content = expectGeneratedTemplate(platform, key);
      expectTaskArtifactContract(content);
    });
  }
...
describe("regression: configSectionsAdded (issue-codex-dispatch-mode)", () => {
  ...
  it("[config-sections] applyConfigSectionsAdded appends section when sentinel missing, idempotent on rerun", async () => {
    ...
    const trellisDir = path.join(tmpDir, ".cviauto");
    ...
    const bundled = new Map<string, string>([
      [".cviauto/config.yaml", bundledTemplate],
    ]);
  ...
  it("[config-sections] applyConfigSectionsAdded skips when target file does not exist", async () => {
    ...
    const result = applyConfigSectionsAdded(
      ...
      tmpDir,
      new Map<string, string>([[".cviauto/config.yaml", "# fake template"]]),
    );
  ...
  it("[config-sections] manifest 0.5.7 declares the codex dispatch_mode section", () => {
    ...
    expect(entry?.file).toBe(".cviauto/config.yaml");
...
// =============================================================================
// safe-commit: gitignored .cviauto/ recovery (0.5.10 → 0.5.11)
// =============================================================================
//
// Real user incident: project .gitignore listed `.cviauto/`. add_session.py's
// auto-commit ran `git add .cviauto/workspace .cviauto/tasks`, got `ignored
// by .gitignore`, fell back to a hint suggesting `git add .cviauto &&
// commit`. The AI agent driving the workflow extrapolated that to
// `git add -f .cviauto/`, which forced in `.cviauto/.backup-*/`,
// `.cviauto/worktrees/`, `.cviauto/.template-hashes.json`, etc. — 548 files
...
// That auto-`-f` was an over-fix — when a user gitignores `.cviauto/` they
// mean "keep .cviauto/ local-only", and forcing the commit through (even on
// narrow paths) violates user intent. Group-chat report: a finish-work auto
// committed `.cviauto/workspace/` straight into a repo whose .gitignore
// excluded `.cviauto/`.
//
// 0.5.11 fix (current):
//   - Plain `git add <specific>` is tried once. On `ignored by`, the script
//     warns and skips the auto-commit — never `-f`.
//   - `session_auto_commit: false` is the default and opts the user out of
//     auto-stage and auto-commit entirely (issue #245).
//   - The warning explicitly says ``Do NOT use `git add -f .cviauto/```` so
//     AI re-reading the log doesn't reinvent the bug, and points at the
//     `session_auto_commit: false` knob.
//
// These tests synthesize a tmp git repo with `.cviauto/` gitignored and
// verify (a) the default writes local files but skips git entirely,
// (b) explicit `session_auto_commit: true` still warns + skips on `ignored by`
// (no commit, no -f), and (c) the negative-rule warning + config hint are
// reachable.
// =============================================================================

describe("regression: safe auto-commit when .cviauto/ is gitignored (0.5.10 → 0.5.11)", () => {
  let tmpDir: string;
  ...
  function writeWorkspaceIndex(): void {
    writeFile(
      ".cviauto/workspace/test-dev/index.md",
      [
  ...
  function setupRepo(options?: { gitignoreTrellis?: boolean }): void {
    ...
    writeFile(
      ".cviauto/workspace/test-dev/journal-1.md",
      "# Journal - test-dev (Part 1)\n\n---\n",
    );
    writeWorkspaceIndex();
    ...
    writeFile(
      ".cviauto/.backup-2026-05-09/should-not-be-committed.txt",
      "secret-backup\n",
    );
    writeFile(
      ".cviauto/worktrees/wt-a/should-not-be-committed.txt",
      "secret-worktree\n",
    );
    writeFile(
    ...
    writeFile(
      ".cviauto/.runtime/sessions/should-not-be-committed.json",
      "{}\n",
    );

    if (options?.gitignoreTrellis) {
      writeFile(".gitignore", ".cviauto/\n");
    }
  ...
  it("[session_auto_commit default false] add_session skips git even when .cviauto/ is ignored", () => {
    ...
    expect(stderr).not.toContain("ignored by your .gitignore");

    // Nothing under .cviauto/ should be tracked: the user's .gitignore
    // intent is preserved.
    const tracked = listCommittedFiles();
    for (const tracked_path of tracked) {
      expect(
        tracked_path.startsWith(".cviauto/"),
        `should not commit anything under .cviauto/ (got: ${tracked_path})`,
      ).toBe(false);
    }

    // The journal + index files are still on disk — only git was untouched.
    expect(
      fs.existsSync(
        path.join(tmpDir, ".cviauto/workspace/test-dev/journal-1.md"),
      ),
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".cviauto/workspace/test-dev/index.md")),
    ).toBe(true);
  });

  it("[session_auto_commit=true] add_session warns and skips when .cviauto/ is ignored", () => {
    setupRepo({ gitignoreTrellis: true });
    writeFile(".cviauto/config.yaml", "session_auto_commit: true\n");
    const { stderr } = runAddSession();

    // Plain add fails with "ignored by". The script must NOT retry with -f.
    expect(stderr).not.toContain("Auto-committed");
    expect(stderr).toContain("ignored by your .gitignore");
    expect(stderr).toContain("Do NOT use `git add -f .cviauto/`");
    expect(stderr).toContain("session_auto_commit: false");
  ...
  it("[session_auto_commit=true] add_session auto-commits when .cviauto/ is NOT ignored", () => {
    // Explicit opt-in preserves the legacy auto-commit behavior for users
    // whose .gitignore does not exclude .cviauto/.
    ...
    const tracked = listCommittedFiles();
    expect(tracked).toContain(".cviauto/workspace/test-dev/journal-1.md");
  });
  ...
  it("[gitignore-trellis] safe_commit module ships and contains the negative warning + new config hint", () => {
    ...
    expect(safeCommit).toContain("Do NOT use `git add -f .cviauto/`");
  ...
  it("[session_auto_commit default false] task.py archive skips git even when .cviauto/ is ignored", () => {
    setupRepo({ gitignoreTrellis: true });
    // Create a task to archive.
    writeFile(
      ".cviauto/tasks/issue-500/task.json",
      JSON.stringify(
    ...
    writeFile(".cviauto/tasks/issue-500/prd.md", "# PRD\n");
    ...
    const tracked = listCommittedFiles();
    // Nothing under .cviauto/ should be tracked.
    for (const t of tracked) {
      expect(
        t.startsWith(".cviauto/"),
        `should not commit anything under .cviauto/ (got: ${t})`,
      ).toBe(false);
    }
    ...
    const archiveExists = fs
      .readdirSync(path.join(tmpDir, ".cviauto/tasks/archive"))
      .some((monthDir) => {
        const monthPath = path.join(tmpDir, ".cviauto/tasks/archive", monthDir);
        return (
  ...
  function writeConfigYaml(content: string): void {
    writeFile(".cviauto/config.yaml", content);
  }
  ...
  it("[session_auto_commit=false] task.py archive skips git entirely", () => {
    ...
    writeFile(
      ".cviauto/tasks/issue-600/task.json",
      JSON.stringify(
        { title: "Test archive", status: "in_progress", package: null },
        null,
        2,
    ...
    writeFile(".cviauto/tasks/issue-600/prd.md", "# PRD\n");
  ...
  it("[session_auto_commit] inline comment is stripped before parsing", () => {
    ...
    // verifies the helper is shared with cviauto_config.py's parser.
    setupRepo({ gitignoreTrellis: false });
  ...
  it("[session_auto_commit] invalid value falls back to false with stderr warn", () => {
    ...
    // Falls back to false → local files are written but git is untouched.
    expect(stderr).toContain("session_auto_commit: false");
    expect(stderr).not.toContain("Auto-committed");
