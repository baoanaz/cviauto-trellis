...
describe("trae shared skills (from common source)", () => {
  ...
  it("does not include platform-specific syntax in resolved output", () => {
    const skills = resolveAllAsSkills(AI_TOOLS.trae.templateContext);
    for (const skill of skills) {
      // Trae uses /cviauto- prefix, not /cviauto:
      expect(skill.content).not.toContain("/cviauto:");
...
// =============================================================================
//
// cviauto-implement / cviauto-check agent markdown MUST contain a hard recursion
// guard so the dispatched sub-agent does not spawn another
// cviauto-implement / cviauto-check sub-agent.
describe("trae sub-agent recursion guard", () => {
  for (const name of ["cviauto-implement", "cviauto-check"] as const) {
    it(`${name}.md forbids spawning cviauto-implement / cviauto-check`, () => {
      ...
      expect(content).toContain("cviauto-implement");
      expect(content).toContain("cviauto-check");
...
describe("trae pull-based prelude injection (class-2)", () => {
  it("applyPullBasedPreludeMarkdown injects context-loading instructions", () => {
    ...
    for (const agent of agents) {
      if (
        agent.name === "cviauto-implement" ||
        agent.name === "cviauto-check"
      ) {
        expect(agent.content).toContain("Load Cviauto Context First");
  ...
  it("research agent does not get the prelude (no task context needed)", () => {
    const agents = applyPullBasedPreludeMarkdown(getAllAgents());
    const research = agents.find((a) => a.name === "cviauto-research");
    expect(research).toBeDefined();
    if (!research) return; // guard for type-safety
    expect(research.content).not.toContain("Load Cviauto Context First");
  });
});
