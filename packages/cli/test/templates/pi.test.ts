...
function createMinimalTrellisRoot(): string {
  ...
  mkdirSync(join(root, ".cviauto", "scripts"), { recursive: true });
  writeFileSync(
    join(root, ".cviauto", "workflow.md"),
    [
  ...
  writeFileSync(
    join(root, ".cviauto", "scripts", "get_context.py"),
    [
      "#!/usr/bin/env python3",
      "import sys",
...
describe("pi templates", () => {
  it("provides the three Cviauto sub-agent definitions", () => {
  ...
  it("settings keep Pi-owned skills until shared Agent Skills are platform-neutral", () => {
    ...
    expect(settings.extensions).toEqual(["./extensions/cviauto/index.ts"]);
    expect(settings.skills).toEqual(["./skills"]);
    expect(settings.prompts).toEqual(["./prompts"]);
    expect(settings.packages).toBeUndefined();
  });

  it("collects a manual cviauto-start prompt for Pi fallback bootstrap", () => {
    const templates = collectPiTemplates();

    expect(templates.get(".pi/prompts/cviauto-start.md")).toContain(
      "# Start Session",
    );
    expect(templates.get(".pi/prompts/cviauto-continue.md")).toContain(
      "get_context.py --mode phase",
    );
    expect(templates.get(".pi/prompts/cviauto-finish-work.md")).toContain(
      "finish-work",
    );
  });

  it("extension registers the cviauto_subagent tool with mode+thinking schema", () => {
    ...
    expect(extension).toContain('label: "Cviauto Subagent"');
  ...
  it("extension wires the Pi events Cviauto needs for context flow", () => {
    const extension = getExtensionTemplate();
    ...
    // before_agent_start: inject Cviauto task context + per-turn breadcrumb
  ...
  it("injects workflow state on input and startup context on first agent start", () => {
    ...
    expect(first.systemPrompt).toContain(
      "Cviauto compact SessionStart context",
    );
    expect(first.systemPrompt).toContain("<first-reply-notice>");
    expect(first.systemPrompt).toContain("<cviauto-workflow>");
    expect(first.systemPrompt).toContain("Phase 1: Plan");
    expect(first.systemPrompt).toContain("No active Cviauto task found");
  ...
  it("extension tool_result handler marks failed/cancelled subagent runs as errors", () => {
    ...
    expect(extension).toContain('ev.toolName === "cviauto_subagent"');
  ...
  it("normalizeAgent prefixes bare names with trellis- and leaves prefixed names alone", () => {
    ...
    expect(normalizeAgent("implement")).toBe("cviauto-implement");
    expect(normalizeAgent("check")).toBe("cviauto-check");
    expect(normalizeAgent("cviauto-research")).toBe("cviauto-research");
    expect(normalizeAgent(undefined)).toBe("cviauto-implement");
  ...
  it("isTrellisAgent gates on a real .pi/agents/*.md definition file", () => {
    ...
    writeFileSync(
      join(root, ".pi", "agents", "cviauto-implement.md"),
      "---\nname: cviauto-implement\n---\n",
    );

    expect(isTrellisAgent(root, "cviauto-implement")).toBe(true);
