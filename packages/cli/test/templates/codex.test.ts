...
// =============================================================================
//
// cviauto-implement / cviauto-check agent toml MUST contain a hard recursion
// guard that tells the sub-agent it is already the dispatched agent and must
// not spawn another cviauto-implement / cviauto-check sub-agent. Without this,
// SessionStart's "dispatch cviauto-implement" guidance leaks into sub-agent
...
describe("codex sub-agent recursion guard (issue #234)", () => {
  for (const name of ["cviauto-implement", "cviauto-check"] as const) {
    it(`${name}.toml developer_instructions forbids spawning cviauto-implement / cviauto-check`, () => {
...
describe("codex session-start.py compact SessionStart context", () => {
  ...
  it("uses compact task artifact guidance instead of sub-agent dispatch prose", () => {
    ...
    expect(content).toContain("Cviauto compact SessionStart context");
