...
import {
  ...
  isTrellisSubagent,
  TrellisContext,
} from "../../src/templates/opencode/lib/cviauto-context.js";
import {
  buildSessionContext,
...
async function createOpenCodeInjectHooks(
  ...
  return (await injectSubagentContextPlugin({
    directory: "/tmp/cviauto-opencode-test",
    platform,
    env,
...
describe("opencode session-start history detection", () => {
  it("includes the one-shot first-reply notice in injected context", () => {
    ...
    expect(context).toContain("Cviauto SessionStart context is loaded");
...
describe("opencode bash session context", () => {
  it("injects TRELLIS_CONTEXT_ID into Bash commands from plugin sessionID", async () => {
    ...
    const output = {
      args: {
        command: "python3 ./.cviauto/scripts/task.py start .cviauto/tasks/demo",
      },
    };
    ...
    expect(output.args.command).toBe(
      "export TRELLIS_CONTEXT_ID='opencode_oc-a'; python3 ./.cviauto/scripts/task.py start .cviauto/tasks/demo",
    );
  });
  ...
  it("uses PowerShell environment syntax on Windows", async () => {
    ...
    const output = {
      args: {
        command: "python ./.cviauto/scripts/task.py start .cviauto/tasks/demo",
      },
    };
    ...
    expect(output.args.command).toBe(
      "$env:TRELLIS_CONTEXT_ID = 'opencode_oc-a'; python ./.cviauto/scripts/task.py start .cviauto/tasks/demo",
    );
  });
  ...
  it("does not duplicate an explicit TRELLIS_CONTEXT_ID assignment", async () => {
    ...
    const output = {
      args: {
        command:
          "TRELLIS_CONTEXT_ID=manual python3 ./.cviauto/scripts/task.py current",
      },
    };
  ...
  it("does not duplicate an explicit exported TRELLIS_CONTEXT_ID", async () => {
    ...
    const output = {
      args: {
        command:
          "export TRELLIS_CONTEXT_ID=manual; python3 ./.cviauto/scripts/task.py current",
      },
    };
  ...
  it("does not duplicate an explicit env TRELLIS_CONTEXT_ID assignment", async () => {
    ...
    const output = {
      args: {
        command:
          "env FOO=bar TRELLIS_CONTEXT_ID=manual python3 ./.cviauto/scripts/task.py current",
      },
    };
  ...
  it("does not duplicate an explicit PowerShell TRELLIS_CONTEXT_ID assignment", async () => {
    ...
    const output = {
      args: {
        command:
          "$env:TRELLIS_CONTEXT_ID = 'manual'; python ./.cviauto/scripts/task.py current",
      },
    };
...
function setupTrellisProject(): string {
  ...
  const taskDir = join(dir, ".cviauto", "tasks", "demo-task");
  mkdirSync(taskDir, { recursive: true });
  mkdirSync(join(dir, ".cviauto", ".runtime", "sessions"), { recursive: true });
  writeFileSync(
    join(taskDir, "prd.md"),
  ...
  writeFileSync(
    join(dir, ".cviauto", "workflow.md"),
    [
      "# Workflow",
      "",
      "[workflow-state:in_progress]",
      "Active task: <task path>. Dispatch cviauto-implement or cviauto-check.",
      "[/workflow-state:in_progress]",
      "",
    ].join("\n"),
  );
...
function writeSessionFile(dir: string, key: string, taskRef: string): void {
  const file = join(dir, ".cviauto", ".runtime", "sessions", `${key}.json`);
...
describe("opencode subagent helper", () => {
  it("isTrellisSubagent matches the three trellis sub-agent names", () => {
    expect(isTrellisSubagent({ agent: "cviauto-implement" })).toBe(true);
    expect(isTrellisSubagent({ agent: "cviauto-check" })).toBe(true);
    expect(isTrellisSubagent({ agent: "cviauto-research" })).toBe(true);
  });

  it("isTrellisSubagent rejects unrelated agents", () => {
    expect(isTrellisSubagent({ agent: "build" })).toBe(false);
    expect(isTrellisSubagent({ agent: "cviauto-implement-extra" })).toBe(false);
...
describe("opencode TrellisContext single-session fallback", () => {
  ...
  it("returns the only session file when exactly one exists", () => {
    writeSessionFile(dir, "opencode_sole", ".cviauto/tasks/demo-task");
    const ctx = new TrellisContext(dir);
    const active = ctx.getActiveTask({ sessionID: "missing-key" });

    expect(active.taskPath).toBe(".cviauto/tasks/demo-task");
  ...
  it("refuses to guess when two or more session files exist", () => {
    writeSessionFile(dir, "opencode_a", ".cviauto/tasks/demo-task");
    writeSessionFile(dir, "opencode_b", ".cviauto/tasks/demo-task");
    const ctx = new TrellisContext(dir);
    const active = ctx.getActiveTask({ sessionID: "missing-key" });
  ...
  it("prefers an exact context-key match over the fallback", () => {
    writeSessionFile(dir, "opencode_session_exact", ".cviauto/tasks/demo-task");
    writeSessionFile(dir, "opencode_other", ".cviauto/tasks/demo-task");
    const ctx = new TrellisContext(dir);
...
describe("opencode inject-subagent-context (issue #264)", () => {
  ...
  it("inlines JSONL-referenced spec content into the implement prompt", async () => {
    ...
    const specPath = join(dir, ".cviauto", "spec", "demo.md");
    mkdirSync(join(dir, ".cviauto", "spec"), { recursive: true });
    writeFileSync(specPath, "# Demo Spec\n\nUNIQUE_SPEC_MARKER_42");
    writeFileSync(
      join(dir, ".cviauto", "tasks", "demo-task", "implement.jsonl"),
      JSON.stringify({ file: ".cviauto/spec/demo.md", reason: "test" }) + "\n",
    );
    writeSessionFile(dir, "opencode_sole", ".cviauto/tasks/demo-task");
    ...
    expect(output.args.prompt).toContain("<!-- trellis-hook-injected -->");
    expect(output.args.prompt).toContain("=== .cviauto/spec/demo.md ===");
  ...
  it("mutates check prompt using Active task hint when runtime resolution fails", async () => {
    ...
    const output: TaskToolOutput = {
      args: {
        subagent_type: "cviauto-check",
        prompt: "Active task: .cviauto/tasks/demo-task\n\nplease check",
      },
    };
  ...
  it("Active task hint takes precedence over single-session fallback", async () => {
    ...
    writeSessionFile(dir, "opencode_sole", ".cviauto/tasks/another-task");
    const hintTask = join(dir, ".cviauto", "tasks", "hint-task");
    ...
    const output: TaskToolOutput = {
      args: {
        subagent_type: "cviauto-implement",
        prompt: "Active task: .cviauto/tasks/hint-task\n\ngo",
      },
    };
  ...
  it("emits the trellis-hook-injected marker for research agent too", async () => {
    const output: TaskToolOutput = {
      args: {
        subagent_type: "cviauto-research",
...
describe("opencode chat.message subagent skip (issue #264)", () => {
  ...
  it("session-start.js early-returns when input.agent is a trellis sub-agent", async () => {
    ...
    await hooks["chat.message"](
      { sessionID: "subagent-session", agent: "cviauto-implement" },
      { parts },
    );
  ...
  it("session-start.js skips cviauto-check and cviauto-research", async () => {
    const hooks = (await sessionStartPlugin({
      directory: dir,
      client: undefined,
    })) as ChatMessageHooks;

    for (const agent of ["cviauto-check", "cviauto-research"]) {
