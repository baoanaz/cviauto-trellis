...
const EXPECTED_AGENT_NAMES = ["cviauto-check", "cviauto-implement"];
...
describe("collectReasonixTemplates", () => {
  it("writes both subagent skills under .reasonix/skills/", () => {
    ...
    expect(files.has(".reasonix/skills/cviauto-check/SKILL.md")).toBe(true);
    expect(files.has(".reasonix/skills/cviauto-implement/SKILL.md")).toBe(true);
  });

  it("does not duplicate cviauto-check / cviauto-implement as workflow skills", () => {
    // Subagent skills replace their common-skill equivalents — workflow skills
    // must not also emit a `.reasonix/skills/cviauto-check/SKILL.md` from the
    // shared resolver, or the bundled subagent variant would be overwritten.
    const files = collectReasonixTemplates();
    const checkPaths = [...files.keys()].filter((p) =>
      p.endsWith("/cviauto-check/SKILL.md"),
    );
    const implementPaths = [...files.keys()].filter((p) =>
      p.endsWith("/cviauto-implement/SKILL.md"),
    );
  ...
  it("subagent SKILL.md content carries runAs: subagent frontmatter", () => {
    const files = collectReasonixTemplates();
    const checkBody = files.get(".reasonix/skills/cviauto-check/SKILL.md");
    const implementBody = files.get(
      ".reasonix/skills/cviauto-implement/SKILL.md",
    );
