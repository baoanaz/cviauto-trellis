...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    ...
    // Create .cviauto directory for hashes file
    fs.mkdirSync(path.join(tmpDir, ".cviauto"), { recursive: true });
  });
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".cviauto", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("returns 0 when no template directories exist", () => {
    // Create .cviauto dir for saving hashes but no template files
    fs.mkdirSync(path.join(tmpDir, ".cviauto"), { recursive: true });
    const count = initializeHashes(tmpDir);
    expect(count).toBe(0);
  });

  it("hashes files in .cviauto/ and tracked platform paths", () => {
    // .cviauto/ is always walked recursively. Platform paths (.claude/, etc.)
    ...
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "scripts", "task.py"),
      "print('hello')",
    );
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".cviauto/scripts/task.py");
  ...
  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "workspace"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "workspace", "data.md"),
      "user data",
    );
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".cviauto", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".cviauto/workspace/data.md");
    expect(hashes).not.toHaveProperty(".cviauto/tasks/task.json");
    expect(count).toBe(0);
  });

  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "spec", "guides"), {
      recursive: true,
    });
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "spec", "frontend"), {
      recursive: true,
    });
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "spec", "backend"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "spec", "guides", "index.md"),
      "# Guides",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "spec", "frontend", "index.md"),
      "# Frontend",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "spec", "backend", "index.md"),
      "# Backend",
    );
    ...
    expect(hashes).not.toHaveProperty(".cviauto/spec/guides/index.md");
    expect(hashes).not.toHaveProperty(".cviauto/spec/frontend/index.md");
    expect(hashes).not.toHaveProperty(".cviauto/spec/backend/index.md");
    expect(count).toBe(0);
  });
  ...
  it("collectFiles returns POSIX-normalized paths (no backslashes)", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "scripts", "common"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".cviauto", "scripts", "common", "task.py"),
      "print('x')",
    );
    ...
    expect(hashes).toHaveProperty(".cviauto/scripts/common/task.py");
  });
  ...
  it("does not exclude generated update-spec skills from hashing", () => {
    ...
    // `.pi/skills/cviauto-update-spec/`. The new model doesn't use that
    ...
    const count = initializeHashes(tmpDir, {
      trackedPaths: new Set([".pi/skills/cviauto-update-spec/SKILL.md"]),
    });
    const hashes = loadHashes(tmpDir);

    expect(hashes).toHaveProperty(".pi/skills/cviauto-update-spec/SKILL.md");
    expect(count).toBe(1);
  });
});
...
describe("cross-platform hash storage (POSIX keys + v2 schema)", () => {
  let tmpDir: string;
  const HASHES_REL = path.join(".cviauto", ".template-hashes.json");
  ...
  it("legacy file is discarded then initializeHashes regenerates v2", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, HASHES_REL),
      JSON.stringify({ ".cviauto\\config.yaml": "deadbeef" }),
    );
    ...
    expect(parsed.hashes).not.toHaveProperty(".cviauto\\config.yaml");
    expect(parsed.hashes).not.toHaveProperty(".cviauto/config.yaml");
    // Newly hashed file is present with POSIX key
    expect(parsed.hashes).toHaveProperty(".cviauto/scripts/task.py");
  });
