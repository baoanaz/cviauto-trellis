...
describe("pruneOrphanManifestKeys", () => {
  ...
  it("preserves every .cviauto/* entry regardless of platform-collect output", () => {
    const hashes = {
      ".cviauto/workflow.md": "h1",
      ".cviauto/scripts/task.py": "h2",
      ".cviauto/config.yaml": "h3",
    };
  ...
  it("does NOT rewrite disk when nothing was pruned", () => {
    const hashes = { ".cviauto/workflow.md": "h1" };
    saveHashes(tmpDir, hashes);

    const hashFile = path.join(tmpDir, ".cviauto", ".template-hashes.json");
