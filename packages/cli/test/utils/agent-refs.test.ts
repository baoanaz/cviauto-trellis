...
describe("collectReferencedAgents", () => {
  it("extracts names from `--agent <name>` flags", () => {
    const body = "Run `cviauto channel spawn --agent implement` then check.";
  ...
  it("extracts names from `.cviauto/agents/<name>.md` literal paths", () => {
    const body = "Defined in .cviauto/agents/architect.md";
    expect(collectReferencedAgents(body)).toEqual(["architect"]);
  });

  it("deduplicates and sorts results across both surface forms", () => {
    const body = `
      cviauto channel spawn --agent implement
      cviauto channel spawn --agent check
      see .cviauto/agents/implement.md for details
...
describe("collectMissingAgents", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(cwd, ".cviauto", "agents"), { recursive: true });
  });
  ...
  it("returns referenced names that are not on disk", () => {
    fs.writeFileSync(
      path.join(cwd, ".cviauto", "agents", "implement.md"),
      "---\nname: implement\n---\n",
    );
  ...
  it("returns [] when every referenced agent exists", () => {
    ...
    fs.writeFileSync(
      path.join(cwd, ".cviauto", "agents", "check.md"),
      "---\nname: check\n---\n",
    );
    const body = "--agent implement --agent check";
  ...
  it("recognizes the nested `<name>/AGENT.md` layout", () => {
    fs.mkdirSync(path.join(cwd, ".cviauto", "agents", "designer"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(cwd, ".cviauto", "agents", "designer", "AGENT.md"),
      "---\nname: designer\n---\n",
    );
