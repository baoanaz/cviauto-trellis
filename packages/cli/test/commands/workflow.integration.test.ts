/**
 * Integration tests for `cviauto workflow` and the init/update hash boundary
 * for non-native workflow selection.
 *
 * Coverage:
 * - `cviauto workflow --template native`: writes bundled content, keeps hash.
 * - `cviauto workflow --template tdd`: writes marketplace content, removes hash.
 * - `cviauto init --workflow tdd`: marketplace content is written, hash removed.
 * - `cviauto update` after switch to tdd does NOT silently restore native.
...
function stubMarketplaceFetch(): void {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: string | URL) => {
      ...
      if (
        url.includes("mindfold-ai/marketplace") &&
      ...
      if (
        url.includes("mindfold-ai/marketplace") &&
        url.endsWith("workflows/tdd/workflow.md")
      ) {
...
describe("cviauto workflow integration", () => {
  let tmpDir: string;
  ...
  it("init --workflow-source resolves custom workflow marketplace content", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL) => {
        ...
        if (url.includes("example/workflows") && url.endsWith("/index.json")) {
          return new Response(JSON.stringify(index), { status: 200 });
        }
        if (
          url.includes("example/workflows") &&
          url.endsWith("workflows/custom/workflow.md")
        ) {
  ...
  it("cviauto workflow --template native refreshes hash after switching from tdd", async () => {
    stubMarketplaceFetch();
    await init({ yes: true, workflow: "tdd" } as Record<string, unknown>);
    expect(loadHashes(tmpDir)[PATHS.WORKFLOW_GUIDE_FILE]).toBeUndefined();
  ...
  it("cviauto workflow --template tdd writes marketplace content and removes the hash", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    expect(loadHashes(tmpDir)[PATHS.WORKFLOW_GUIDE_FILE]).toBeTruthy();
  ...
  it("cviauto update after switching to tdd does not silently restore native workflow", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    await runWorkflowCommand({ template: "tdd" });
