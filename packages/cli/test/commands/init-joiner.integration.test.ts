 ...
 *   no .cviauto/                        → creator bootstrap task
 *   .cviauto/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  function seedDefaultCviautoRegistry(): void {
    ...
    registryDownload.files.set("features/index.md", "# Cviauto Features\n");
    registryDownload.files.set(
      "features/cviauto-example.md",
      "# Cviauto Feature Example\n",
    );
  }
  ...
  /**
   * Helper: simulate a fresh clone of an existing Trellis project — `.cviauto/`
  ...
  /** Helper: simulate same-dev re-init — both `.cviauto/` and `.developer` exist */
  ...
  it("#2 existing .cviauto/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".cviauto/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.cviauto/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(
      `${expectedPythonCmd} ./.cviauto/scripts/task.py finish`,
    );
    expect(prd).toContain(
      `${expectedPythonCmd} ./.cviauto/scripts/task.py archive 00-join-bob --no-commit`,
    );
  ...
  it("#2b issue #204: existing .cviauto/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .cviauto/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .cviauto/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .cviauto/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .cviauto/
  ...
  it("#7 handleReinit path: existing .cviauto/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .cviauto/ + .developer → no task created", async () => {
