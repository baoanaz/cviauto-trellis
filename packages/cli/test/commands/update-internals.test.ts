...
describe("cleanupEmptyDirs", () => {
  ...
  it("[CR#1] does not delete .cviauto root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".cviauto"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".cviauto");
    expect(fs.existsSync(path.join(tmpDir, ".cviauto"))).toBe(true);
  });

  it("recursively cleans parent directories but stops at root", () => {
    // Create .cviauto/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".cviauto", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".cviauto/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(path.join(tmpDir, ".cviauto", "scripts", "multi_agent")),
    ).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cviauto", "scripts"))).toBe(false);
    // .cviauto root must survive
    expect(fs.existsSync(path.join(tmpDir, ".cviauto"))).toBe(true);
  });
...
describe("sortMigrationsForExecution", () => {
  ...
  it("puts rename-dir before rename and delete", () => {
    const items = [
      ...
      { type: "rename-dir" as const, from: ".cviauto/old", to: ".cviauto/new" },
  ...
  it("sorts rename-dir by path depth (deeper first)", () => {
    const items = [
      { type: "rename-dir" as const, from: ".cviauto/a", to: ".cviauto/x" },
      {
        type: "rename-dir" as const,
        from: ".cviauto/a/b/c",
        to: ".cviauto/x/y/z",
      },
      { type: "rename-dir" as const, from: ".cviauto/a/b", to: ".cviauto/x/y" },
    ];
    const sorted = sortMigrationsForExecution(items);
    expect(sorted[0].from).toBe(".cviauto/a/b/c"); // depth 4
    expect(sorted[1].from).toBe(".cviauto/a/b"); // depth 3
    expect(sorted[2].from).toBe(".cviauto/a"); // depth 2
  });
...
describe("shouldExcludeFromBackup", () => {
  ...
  it.each([
    ...
    ".cviauto/.backup-2026-04-22T10-24-27/.opencode/node_modules/zod/index.js",
  ])("excludes dependency tree %s", (p) => {
    expect(shouldExcludeFromBackup(p)).toBe(true);
  });

  it.each([
    ".cviauto/workspace/developer/journal-1.md",
    ".cviauto/tasks/04-17-foo/prd.md",
    ".cviauto/spec/cli/backend/index.md",
    ".cviauto/backlog/idea.md",
    ".cviauto/agent-traces/trace.jsonl",
  ...
  it("excludes previous backups", () => {
    expect(
      shouldExcludeFromBackup(".cviauto/.backup-2026-04-20T01-00-00/x"),
    ).toBe(true);
  });

  it.each([
    ".claude/commands/cviauto/continue.md",
    ".claude/skills/cviauto-check/SKILL.md",
    ".cviauto/workflow.md",
    ".cviauto/scripts/get_context.py",
    ".agents/skills/cviauto-check/SKILL.md",
  ...
  // `cviauto update --migrate`, late April 2026).
  it.each([
    ".claude\\worktrees\\feat-x\\src\\main.ts",
    ".cviauto\\tasks\\04-17-foo\\prd.md",
    ".cviauto\\workspace\\dev\\journal-1.md",
