...
describe("update() integration", () => {
  ...
  beforeEach(() => {
    ...
    // Mock fetch for npm registry and the Cviauto default direct registry probe.
    vi.stubGlobal(
      "fetch",
  ...
  it("[issue-zcode-codex-upgrade] zcode .agents skills do not trigger legacy Codex backfill", async () => {
    ...
    expect(fs.existsSync(projectFile(".zcode/commands/cviauto/start.md"))).toBe(
      true,
    );
    expect(
      fs.existsSync(projectFile(".agents/skills/cviauto-start/SKILL.md")),
    ).toBe(false);
    expect(
      fs.existsSync(projectFile(".agents/skills/cviauto-continue/SKILL.md")),
    ).toBe(false);
  ...
  it("#13 user-added Cviauto feature specs are preserved after update with force", async () => {
    await setupProject();

    // Teams add their own feature specs outside the default registry manifest.
    const featureSpec = path.join(
      tmpDir,
      PATHS.SPEC,
      "features",
      "cviauto-team-feature.md",
    );
    const customContent =
      "# Cviauto Team Feature\n\nOwned by a feature team.\n";
    fs.writeFileSync(featureSpec, customContent);

    await update({ force: true });

    expect(fs.readFileSync(featureSpec, "utf-8")).toBe(customContent);
  });
  ...
  it("#18 safe-file-delete preserves user-modified deprecated file", async () => {
    ...
    const deprecatedDir = path.join(tmpDir, ".claude", "commands", "cviauto");
  ...
  it("#19 safe-file-delete handles missing deprecated files without crash", async () => {
    ...
    // The manifest has safe-file-delete entries for .claude/commands/cviauto/before-backend-dev.md etc.
  ...
  function stageLegacy040Project(): void {
    ...
    const legacyDir = path.join(tmpDir, ".claude", "commands", "cviauto");
  ...
  function clearMigrationTarget(): void {
    fs.rmSync(path.join(tmpDir, ".claude/skills/cviauto-before-dev"), {
      recursive: true,
      force: true,
    });
  }
  ...
  // diff/merge their customizations without digging through .cviauto/.backup-*/.
  ...
  it("#25 backup-rename leaves inline <new-path>.backup with original content", async () => {
    ...
    const legacyPath = path.join(
      tmpDir,
      ".claude/commands/cviauto/before-dev.md",
    );
    ...
    const newPath = path.join(
      tmpDir,
      ".claude/skills/cviauto-before-dev/SKILL.md",
    );
  ...
  it("#26 rename-anyway does NOT leave an inline .backup (relies on project snapshot)", async () => {
    ...
    expect(fs.existsSync(newPath)).toBe(true);
    // No inline .backup — the full-project snapshot under .cviauto/.backup-*
  ...
  it("#workflow-md-r4 updates workflow.md as one runtime template when hash-tracked", async () => {
    ...
    expect(updated).toContain("[Claude Code, OpenCode, codex-sub-agent]");
    expect(updated).toContain("[/Claude Code, OpenCode, codex-sub-agent]");
    expect(updated).toContain("[codex-inline]");
