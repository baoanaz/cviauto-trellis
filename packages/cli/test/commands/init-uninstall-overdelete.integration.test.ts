 ...
 * (.cviauto/tasks/05-13-uninstall-overdelete-manifest-leak).
 *
...
describe("init + uninstall: manifest accuracy + homedir guard", () => {
  ...
  it("#R3.2 uninstall self-heals + preserves user file even without prior update", async () => {
    ...
    // and runs `cviauto uninstall` directly. Prune must fire before plan
  ...
  it("#R3.3 prune keeps migration-referenced paths even if not in collectTemplates", async () => {
    ...
    // CAN assert the prune behavior preserves .cviauto/ entries which is the
    ...
    const hashes = loadHashes(tmpDir);
    hashes[".cviauto/workflow.md"] = "ok";
    saveHashes(tmpDir, hashes);

    await update({});

    // .cviauto/* entries are kept.
    expect(loadHashes(tmpDir)).toHaveProperty(".cviauto/workflow.md");
  });
  ...
  it("#R2.1 init refuses to run when cwd === $HOME", async () => {
    ...
    try {
      ...
      // No .cviauto dir was created.
      expect(fs.existsSync(path.join(fakeHome, ".cviauto"))).toBe(false);
    } finally {
  ...
  it("#R2.3 TRELLIS_ALLOW_HOMEDIR=1 bypasses the guard for init", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(fakeHome, ".cviauto"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
  ...
  it("#R2.4 subdirectories of $HOME are NOT blocked", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(subDir, ".cviauto"))).toBe(true);
    } finally {
      fs.rmSync(fakeHome, { recursive: true, force: true });
    }
  });
