...
describe("registry internal consistency", () => {
  ...
  it("no configDir collides with .cviauto", () => {
    for (const id of PLATFORM_IDS) {
      expect(AI_TOOLS[id].configDir).not.toBe(".cviauto");
    }
  });
...
describe("UserPromptSubmit hook wiring", () => {
  ...
  it("kiro main `cviauto` agent wires userPromptSubmit; sub-agents do not", async () => {
    // Kiro DOES support per-turn hooks (official docs: CLI agent
    // `hooks.userPromptSubmit`). The main `cviauto` agent wires the per-turn
    // breadcrumb; the 3 sub-agents only inject sub-agent context on spawn.
    const { getAllAgents } = await import("../src/templates/kiro/index.js");
    for (const agent of getAllAgents()) {
      const content = agent.content;
      const parsed = JSON.parse(content) as {
        hooks?: Record<string, unknown>;
      };
      if (agent.name === "cviauto") {
        expect(Object.keys(parsed.hooks ?? {})).toContain("userPromptSubmit");
        expect(content).toContain("inject-workflow-state.py");
      } else {
        expect(
          content,
          `kiro/agents/${agent.name} (sub-agent) should not wire inject-workflow-state.py`,
