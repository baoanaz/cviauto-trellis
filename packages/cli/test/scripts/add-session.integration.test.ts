...
function setupRepo(tmp: string): void {
  ...
  const r = spawnSync(
    "python3",
    [".cviauto/scripts/init_developer.py", DEVELOPER],
...
function setCurrentTask(repo: string, taskName: string): void {
  const sessionsDir = path.join(repo, ".cviauto", ".runtime", "sessions");
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, "session.json"),
    JSON.stringify({
      current_task: `.cviauto/tasks/${taskName}`,
      platform: "session",
    }) + "\n",
  );
}
...
function runAddSession(repo: string, title: string): void {
  const r = spawnSync(
    "python3",
    [".cviauto/scripts/add_session.py", "--title", title],
...
describe.skipIf(!hasPython())("add_session.py auto-commit", () => {
  ...
  it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
    ...
    expect(status).toMatch(/\.cviauto\/tasks\/task-b\/prd\.md/);
  });
  ...
  it("does not wide-scan task dirs when the current task is unresolvable (>=2 sessions)", () => {
    ...
    setCurrentTask(tmp, "task-a");
    const sessionsDir = path.join(tmp, ".cviauto", ".runtime", "sessions");
    fs.writeFileSync(
      path.join(sessionsDir, "session2.json"),
      JSON.stringify({
        current_task: ".cviauto/tasks/task-b",
        platform: "session",
      }) + "\n",
    );
    git(tmp, "add", "-A");
    ...
    fs.appendFileSync(
      path.join(tmp, ".cviauto", "tasks", "task-a", "prd.md"),
      "DIRTY A\n",
    );
    fs.appendFileSync(
      path.join(tmp, ".cviauto", "tasks", "task-b", "prd.md"),
    ...
    const status = git(tmp, "status", "--porcelain");
    expect(status).toMatch(/\.cviauto\/tasks\/task-a\/prd\.md/);
    expect(status).toMatch(/\.cviauto\/tasks\/task-b\/prd\.md/);
  });
});
