...
function setupRepo(tmp: string): void {
  ...
  const scriptsDest = path.join(tmp, ".cviauto", "scripts");
  ...
  fs.writeFileSync(
    path.join(tmp, ".cviauto", "config.yaml"),
    "session_auto_commit: true\n",
  );
}

function makeTask(repo: string, name: string, prdBody: string): void {
  const dir = path.join(repo, ".cviauto", "tasks", name);
...
function runArchive(repo: string, taskName: string): void {
  const r = spawnSync(
    "python3",
    [".cviauto/scripts/task.py", "archive", taskName],
...
describe.skipIf(!hasPython())("task.py archive auto-commit", () => {
  let tmp: string;
  ...
  it("does not bundle dirty changes from other task dirs (scope-creep fix)", () => {
    ...
    fs.appendFileSync(
      path.join(tmp, ".cviauto", "tasks", "task-b", "prd.md"),
    ...
    expect(status).toMatch(/M\s+\.cviauto\/tasks\/task-b\/prd\.md/);
  });

  it("stages source-side deletions in the archive commit (phantom-delete fix)", () => {
    makeTask(tmp, "big", "# big task\n");
    // Add many files under research/ to mimic the production case that
    // surfaced the bug.
    const researchDir = path.join(tmp, ".cviauto", "tasks", "big", "research");
    ...
    expect(deletes.every((p) => p.startsWith(".cviauto/tasks/big/"))).toBe(
      true,
    );
  ...
  it("fails when archive auto-commit cannot record tracked source deletes", () => {
    ...
    const r = spawnSync(
      "python3",
      [".cviauto/scripts/task.py", "archive", "tracked"],
    ...
    const status = git(tmp, "status", "--porcelain");
    expect(status).toContain(".cviauto/tasks/tracked/");
    expect(status).toContain(".cviauto/tasks/archive/");
  });
});
