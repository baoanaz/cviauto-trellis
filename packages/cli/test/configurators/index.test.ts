...
describe("ALL_MANAGED_DIRS", () => {
  it("starts with .cviauto", () => {
    expect(ALL_MANAGED_DIRS[0]).toBe(".cviauto");
  });

  it("contains .cviauto plus all managed dirs", () => {
...
describe("isManagedPath", () => {
  ...
  it("matches platform config sub-paths", () => {
    ...
    expect(isManagedPath(".devin/workflows/cviauto-start.md")).toBe(true);
    ...
    expect(isManagedPath(".pi/extensions/cviauto/index.ts")).toBe(true);
    expect(isManagedPath(".pi/prompts/cviauto-continue.md")).toBe(true);
  });
  ...
  it("matches exact managed directory names", () => {
    ...
    expect(isManagedPath(".cviauto")).toBe(true);
  });

  // Positive: .cviauto hardcoded paths
  it("matches .cviauto sub-paths", () => {
    expect(isManagedPath(".cviauto/spec")).toBe(true);
    expect(isManagedPath(".cviauto/tasks/some-task")).toBe(true);
  });
  ...
  it("rejects prefix-similar non-sub-paths", () => {
    ...
    expect(isManagedPath(".cviauto-old")).toBe(false);
  ...
  it("rejects path traversal", () => {
    ...
    expect(isManagedPath("../.cviauto/spec")).toBe(false);
  });
...
describe("isManagedRootDir", () => {
  ...
  it("matches .cviauto", () => {
    expect(isManagedRootDir(".cviauto")).toBe(true);
  });
  ...
  it("rejects sub-paths (not a root dir)", () => {
    ...
    expect(isManagedRootDir(".cviauto/spec")).toBe(false);
  });
...
describe("getPublicInitToolChoices", () => {
  it("exposes only the Cviauto-supported init platforms", () => {
    expect(
      getPublicInitToolChoices().map((choice) => choice.platformId),
...
describe("collectPlatformTemplates", () => {
  ...
  it("tracks bundled built-in skill files for every skill-writing platform", () => {
    for (const [id, skillRoot] of Object.entries(SKILL_ROOTS)) {
      ...
      expect(result?.has(`${skillRoot}/cviauto-meta/SKILL.md`)).toBe(true);
      expect(
        result?.has(
          `${skillRoot}/cviauto-meta/references/local-architecture/overview.md`,
        ),
      ).toBe(true);
      expect(result?.has(`${skillRoot}/cviauto-spec-bootstrap/SKILL.md`)).toBe(
        true,
      );
      expect(
        result?.has(
          `${skillRoot}/cviauto-spec-bootstrap/references/spec-writing.md`,
        ),
      ).toBe(true);
    }
  });
  ...
  it("copilot collectTemplates includes both tracked and discovery config files", () => {
    ...
    expect(result?.has(".github/hooks/cviauto.json")).toBe(true);
  });

  it("pi collectTemplates includes prompts, agents, extension, and settings", () => {
    const result = collectPlatformTemplates("pi");
    expect(result).toBeInstanceOf(Map);
    expect(result?.has(".pi/prompts/cviauto-start.md")).toBe(true);
    expect(result?.has(".pi/prompts/cviauto-finish-work.md")).toBe(true);
    expect(result?.has(".pi/agents/cviauto-implement.md")).toBe(true);
    expect(result?.has(".pi/extensions/cviauto/index.ts")).toBe(true);
