...
const BUNDLED_SKILL_NAME = "cviauto-meta";
...
describe("configurePlatform", () => {
  ...
  it("configurePlatform('codex') writes shared skill templates from common source", async () => {
    ...
    // `cviauto-start` is included via `resolveAllAsSkillsNeutral` directly —
    ...
    expect(
      fs.existsSync(path.join(skillsRoot, "cviauto-start", "SKILL.md")),
    ).toBe(true);
  });
  ...
  it("configurePlatform('codex') writes custom agents and config", async () => {
    ...
    for (const agent of expectedAgents) {
      ...
      const needsPrelude = ["cviauto-implement", "cviauto-check"].includes(
        agent.name,
      );
      if (needsPrelude) {
        expect(written).toContain("Required: Load Cviauto Context First");
  ...
  it("configurePlatform('kiro') writes main agent, IDE hook, and shared hooks", async () => {
    ...
    // resolved.
    const trellisPath = path.join(tmpDir, ".kiro", "agents", "cviauto.json");
    ...
    expect(trellis.resources).toContain("file://.cviauto/workflow.md");
    ...
    const ideHookPath = path.join(hooksDir, "cviauto-workflow-state.kiro.hook");
  ...
  it("configurePlatform('gemini') writes TOML commands + SKILL.md skills", async () => {
    ...
    // Commands as TOML
    const commandsDir = path.join(tmpDir, ".gemini", "commands", "cviauto");
    ...
    for (const dir of skillDirs) {
      expect(dir.name.startsWith("cviauto-")).toBe(true);
  ...
  it("configurePlatform('qoder') writes commands + skills with the correct split", async () => {
    ...
    expect(actualCommandFiles).toEqual(
      expectedCommands.map((c) => `cviauto-${c.name}.md`).sort(),
    );
    for (const cmd of expectedCommands) {
      const name = `cviauto-${cmd.name}`;
    ...
    expect(actualSkillDirs).not.toContain("cviauto-finish-work");
    expect(actualSkillDirs).not.toContain("cviauto-continue");
    expect(actualSkillDirs).not.toContain("cviauto-start");
  });
  ...
  it("configurePlatform('zcode') keeps command fallbacks out of shared .agents skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".zcode", "commands", "cviauto", "start.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(templates?.has(".zcode/commands/cviauto/start.md")).toBe(true);
    expect(templates?.has(".agents/skills/cviauto-start/SKILL.md")).toBe(false);
    expect(templates?.has(".agents/skills/cviauto-continue/SKILL.md")).toBe(
      false,
    );
    expect(templates?.has(".agents/skills/cviauto-check/SKILL.md")).toBe(true);
  });
  ...
  it("configurePlatform('codebuddy') writes all command templates from common source", async () => {
    ...
    const commandsDir = path.join(tmpDir, ".codebuddy", "commands", "cviauto");
    expect(fs.existsSync(commandsDir)).toBe(true);
  ...
  it("configurePlatform('copilot') writes both tracked and discovery hooks config", async () => {
    ...
    const discovery = path.join(tmpDir, ".github", "hooks", "cviauto.json");
  ...
  it("configurePlatform('droid') creates .factory/commands/cviauto directory", async () => {
    await configurePlatform("droid", tmpDir);
    expect(
      fs.existsSync(path.join(tmpDir, ".factory", "commands", "cviauto")),
    ).toBe(true);
  });
  ...
  it("droid configuration writes commands + skills", async () => {
    ...
    expect(content).toContain("name: cviauto-check");
  });
  ...
  it("configurePlatform('pi') creates extension-backed Pi assets", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".pi", "prompts", "cviauto-finish-work.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "prompts", "cviauto-continue.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "prompts", "cviauto-start.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".pi", "skills", "cviauto-check", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "cviauto-implement.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "cviauto-check.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "cviauto-research.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(extension).toContain('"## Cviauto Agent Definition"');
    ...
    // get_context.py is allowed: it lives in .cviauto/scripts/ and is the
  ...
  it("configurePlatform('pi') writes tracked templates exactly", async () => {
    ...
    for (const agent of getPiAgents()) {
      ...
      if (["cviauto-implement", "cviauto-check"].includes(agent.name)) {
  ...
  it("collectPlatformTemplates('pi') maps prompts, skills, agents, extension, and settings", () => {
    ...
    expect(templates?.get(".pi/prompts/cviauto-start.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/cviauto-finish-work.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/cviauto-continue.md")).toBeDefined();
    expect(templates?.get(".pi/skills/cviauto-check/SKILL.md")).toBeDefined();
    expect(
      templates?.get(
        ".pi/skills/cviauto-meta/references/local-architecture/overview.md",
      ),
    ).toBeDefined();
    expect(
      templates?.get(
        ".pi/skills/cviauto-spec-bootstrap/references/spec-writing.md",
      ),
    ).toBeDefined();
    expect(templates?.get(".pi/agents/cviauto-implement.md")).toContain(
      "Required: Load Cviauto Context First",
    );
    expect(templates?.get(".pi/extensions/cviauto/index.ts")).toBe(
  ...
  it("collectPlatformTemplates('droid') maps commands under .factory/commands/cviauto/", () => {
    const templates = collectPlatformTemplates("droid");
    expect(templates).toBeInstanceOf(Map);
    // Droid is agent-capable → start.md not emitted.
    expect(
      templates?.get(".factory/commands/cviauto/start.md"),
    ).toBeUndefined();
    expect(
      templates?.get(".factory/commands/cviauto/finish-work.md"),
    ).toBeDefined();
    expect(
      templates?.get(".factory/commands/cviauto/continue.md"),
    ).toBeDefined();
  });
  ...
  it("collectPlatformTemplates('copilot') includes tracked + discovery hooks config", () => {
    ...
    expect(templates?.get(".github/hooks/cviauto.json")).toBe(
      resolvePlaceholders(getCopilotHooksConfig()),
    );
  });
