...
describe("replacePythonCommandLiterals", () => {
  ...
  it("handles multiline content with mixed shebangs and commands", () => {
    ...
    const input = [
      ...
      'exec python3 "$0" "$@"',
      "python3 ./.cviauto/scripts/task.py",
    ].join("\n");
    const expected = [
      "#!/usr/bin/env python3",
      "# comment about python",
      'exec python "$0" "$@"',
      "python ./.cviauto/scripts/task.py",
    ].join("\n");
...
describe("resolvePlaceholders", () => {
  ...
  describe("{{CMD_REF:name}}", () => {
    it("resolves with /cviauto: prefix (Claude)", () => {
      const result = resolvePlaceholders(
        "See {{CMD_REF:brainstorm}} for details",
        claudeCtx,
      );
      expect(result).toBe("See /cviauto:brainstorm for details");
    });
    ...
    it("resolves with /cviauto- prefix (Cursor)", () => {
      const result = resolvePlaceholders(
        "Use {{CMD_REF:finish-work}} when done",
        cursorCtx,
      );
      expect(result).toBe("Use /cviauto-finish-work when done");
    });
    ...
    it("handles multiple CMD_REF in one template", () => {
      ...
      expect(resolvePlaceholders(input, claudeCtx)).toBe(
        "/cviauto:start then /cviauto:brainstorm then /cviauto:check",
      );
    });
  ...
  describe("simple substitutions", () => {
    ...
    it("resolves {{PYTHON_CMD}} alongside context placeholders", () => {
      const result = resolvePlaceholders(
        "{{PYTHON_CMD}} ./.cviauto/scripts/task.py and {{CMD_REF:start}}",
        claudeCtx,
      );
      const py = process.platform === "win32" ? "python" : "python3";
      expect(result).toBe(
        `${py} ./.cviauto/scripts/task.py and /cviauto:start`,
      );
    });
  });
  ...
  describe("{{CLI_FLAG}}", () => {
    ...
    it("works alongside {{PYTHON_CMD}} in a realistic init-context invocation", () => {
      const input =
        '{{PYTHON_CMD}} ./.cviauto/scripts/task.py init-context "$TASK_DIR" <type> --platform {{CLI_FLAG}}';
      const py = process.platform === "win32" ? "python" : "python3";
      expect(resolvePlaceholders(input, codexCtx)).toBe(
        `${py} ./.cviauto/scripts/task.py init-context "$TASK_DIR" <type> --platform codex`,
      );
    });
  });
...
describe("resolvePlaceholdersNeutral", () => {
  it("renders {{CMD_REF:name}} as `name` (Cviauto command) — platform-neutral", () => {
    expect(
      resolvePlaceholdersNeutral("See {{CMD_REF:brainstorm}}", claudeCtx),
    ).toBe("See `brainstorm` (Cviauto command)");
    expect(
...
describe("resolveSkillsNeutral / resolveAllAsSkillsNeutral", () => {
  ...
  it("resolveSkillsNeutral renders CMD_REF without platform-specific prefix", () => {
    ...
    // Claude `/cviauto:name`, Cursor `/cviauto-name`).
