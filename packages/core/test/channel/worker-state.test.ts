...
describe("reduceWorkerRegistry", () => {
  ...
  it("sets idleSince on spawn and clears it mid-turn", () => {
    ...
    expect(reg.workers[0].idleSince).toBe("2026-05-14T00:00:01.000Z");
    expect(reg.workers[0].activity).toBe("idle");
...
describe("matchesInboxPolicy", () => {
  ...
  it("explicitOnly delivers only targeted messages", () => {
    expect(matchesInboxPolicy(msg({ to: "w" }), "w", "explicitOnly")).toBe(
      true,
    );
    expect(
      matchesInboxPolicy(msg({ to: ["a", "w"] }), "w", "explicitOnly"),
    ).toBe(true);
  ...
  it("broadcastAndExplicit also delivers broadcasts", () => {
    ...
    expect(
      matchesInboxPolicy(msg({ to: "w" }), "w", "broadcastAndExplicit"),
    ).toBe(true);
    expect(
