...
> Package boundary and coding rules for `@baoanaz/cviauto-core` and the CLI.
...
|---|---|
| `@baoanaz/cviauto-core` | Reusable domain logic, storage primitives, reducers, task APIs, channel APIs, and typed contracts. |
| `@baoanaz/cviauto` | CLI argument parsing, terminal rendering, command wiring, process exit behavior, template installation, migrations, and release scripts. |
