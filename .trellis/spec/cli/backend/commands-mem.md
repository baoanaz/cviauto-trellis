...
The retrieval engine lives in `@baoanaz/cviauto-core/mem` (`packages/core/src/mem/`);
...
`commands/mem.ts:parseArgv`, then calls the matching core `@baoanaz/cviauto-core/mem`
...
`core/mem/` does **not** use `zod` — `@baoanaz/cviauto-core` keeps a
...
- `packages/core/src/mem/index.ts` — `@baoanaz/cviauto-core/mem` public surface
