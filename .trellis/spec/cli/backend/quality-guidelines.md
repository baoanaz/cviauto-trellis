...
| Kind | Example | Action |
|------|---------|--------|
| **Schema / creator** | `task_store.cmd_create`, `@baoanaz/cviauto-core/task:emptyTaskRecord` (re-exported by `utils/task-json.ts:emptyTaskJson` for legacy CLI call sites) | Drop field from output |
...
`@baoanaz/cviauto-core/task` as `TrellisTaskRecord` +
