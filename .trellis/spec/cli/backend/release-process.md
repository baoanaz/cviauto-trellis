...
|---|---|---|
| `@baoanaz/cviauto` | User-facing CLI | GitHub Actions only |
| `@baoanaz/cviauto-core` | Programmatic core APIs used by the CLI and external integrations | GitHub Actions only |
...
| Packed dependency | Published CLI package must depend on `@baoanaz/cviauto-core` with the exact release version. |
...
8. publish `@baoanaz/cviauto-core` if missing
9. publish `@baoanaz/cviauto` if missing
