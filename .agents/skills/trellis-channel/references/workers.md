...
- `--agent <name>` — 加载 `.trellis/agents/<name>.md`（provider/model/as 等系统提示默认值）。
...
`--agent <name>` 解析到 `.trellis/agents/<name>.md`。卡片名称必须匹配 `[A-Za-z0-9._-]+`。Trellis 默认安装附带两张卡片：

- `.trellis/agents/check.md` — 代码质量审查（code-quality reviewer）。
- `.trellis/agents/implement.md` — 用于实现任务的编码 worker。
...
- `--jsonl <path>` — 可重复的 Trellis 清单，每行为 `{"file":"<path>","reason":"<why>"}`。reason 会作为头部注释保留在每个文件内容之前。
...
要审计当前状态，可通过 `channel list`（`WORKERS` 列）列出 worker，并检查 `~/.trellis/channels/<bucket>/<channel>/` 下的 `pid` / `worker-pid` 附属文件。
